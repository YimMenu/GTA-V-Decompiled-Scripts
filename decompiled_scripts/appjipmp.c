void __EntryFunction__()
{
	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_14 = 0.001f;
	iLocal_17 = -1;
	sLocal_18 = "NULL";
	fLocal_23 = 80f;
	fLocal_24 = 140f;
	fLocal_25 = 180f;
	fLocal_28 = 0f;
	fLocal_32 = -0.0375f;
	fLocal_33 = 0.17f;
	iLocal_36 = 3;
	iLocal_40 = 1;
	iLocal_41 = 65;
	iLocal_42 = 49;
	iLocal_43 = 64;
	fLocal_62 = ((0.05f + 0.275f) - 0.01f);
	iLocal_230 = -1;
	fLocal_232 = 3f;
	fLocal_233 = 0f;
	fLocal_234 = 2f;
	fLocal_235 = 100f;
	fLocal_243 = 0.5f;
	StringCopy(&cLocal_250, "CELL_37", 16);
	StringCopy(&Local_254, "CELL_249", 16);
	StringCopy(&Local_258, "CELL_206", 16);
	StringCopy(&Local_262, "CELL_212", 16);
	StringCopy(&Local_266, "CELL_213", 16);
	StringCopy(&Local_270, "", 24);
	iLocal_277 = 6;
	iLocal_290 = -1;
	iLocal_292 = -1;
	iLocal_293 = -1;
	GAMEPLAY::NETWORK_SET_SCRIPT_IS_SAFE_FOR_NETWORK_GAME();
	iLocal_278 = func_300(&uLocal_330, 0);
	if (iLocal_278 > 0)
	{
	}
	func_299();
	func_298(&uLocal_336);
	if (func_297())
	{
		if (func_296())
		{
			func_295();
		}
		else
		{
			func_271(1);
			if ((((((((((((((((func_270() == 35 || func_270() == 33) || func_270() == 37) || func_270() == 21) || func_270() == 40) || func_270() == 42) || func_270() == 44) || func_270() == 46) || func_270() == 48) || func_270() == 50) || func_270() == 52) || func_270() == 54) || func_270() == 56) || func_270() == 58) || func_270() == 60) || func_270() == 62) || func_270() == 64)
			{
				func_269();
			}
			func_268();
			if ((((NETWORK::NETWORK_IS_IN_TRANSITION() || NETWORK::NETWORK_IS_TRANSITION_STARTED()) || NETWORK::NETWORK_IS_TRANSITION_BUSY()) || NETWORK::NETWORK_IS_TRANSITION_MATCHMAKING()) || NETWORK::_0xC571D0E77D8BBC29())
			{
				NETWORK::NETWORK_BAIL_TRANSITION(6, 0, 0);
			}
			func_266();
			func_265();
			func_264();
			func_268();
		}
	}
	if (!PED::IS_PED_INJURED(PLAYER::PLAYER_PED_ID()) && func_262(PLAYER::PLAYER_PED_ID()))
	{
		iLocal_282 = 1;
	}
	Global_2451426.f_1.f_2802 = 0;
	func_259();
	iLocal_276 = 0;
	while (true)
	{
		SYSTEM::WAIT(0);
		if (Local_283 == 0)
		{
			if (func_240())
			{
				Local_283 = 1;
			}
		}
		func_236(0);
		iLocal_278 = func_300(&uLocal_330, 0);
		if (!Global_19486.f_1 == 9 && Global_19486.f_1 > 3)
		{
			switch (iLocal_249)
			{
				case 0:
					func_231();
					break;
				
				case 2:
					func_229();
					break;
				
				case 1:
					func_222();
					break;
				
				case 3:
					func_4();
					break;
				
				default:
					break;
			}
			if (Global_19486.f_1 != 8)
			{
				if (func_3())
				{
					func_295();
				}
			}
			else if (func_2(2, Global_19456, 0))
			{
			}
		}
		if (func_1())
		{
			func_295();
		}
	}
}

int func_1()
{
	if (((Global_19486.f_1 == 1 || Global_19486.f_1 == 3) || Global_19486.f_1 == 0) || Global_19430 == 1)
	{
		Global_19473 = 1;
		return 1;
	}
	return 0;
}

int func_2(int iParam0, int iParam1, int iParam2)
{
	if (CONTROLS::IS_CONTROL_JUST_PRESSED(iParam0, iParam1) || (iParam2 == 1 && CONTROLS::IS_DISABLED_CONTROL_JUST_PRESSED(iParam0, iParam1)))
	{
		if (GAMEPLAY::IS_PC_VERSION())
		{
			if (GAMEPLAY::UPDATE_ONSCREEN_KEYBOARD() == 0 || (NETWORK::_NETWORK_IS_TEXT_CHAT_ACTIVE() && CONTROLS::_IS_INPUT_DISABLED(2)))
			{
				return 0;
			}
		}
		if (UI::IS_PAUSE_MENU_ACTIVE() || UI::IS_WARNING_MESSAGE_ACTIVE())
		{
			return 0;
		}
		else
		{
			return 1;
		}
	}
	return 0;
}

int func_3()
{
	if (Global_7962 == 1 || Global_19486.f_1 < 7)
	{
		Global_19473 = 1;
		return 1;
	}
	return 0;
}

void func_4()
{
	if (func_221())
	{
		bLocal_279 = false;
		func_215(0);
		return;
	}
	if (iLocal_288 < 0 || iLocal_288 >= 29)
	{
		if (Global_19486.f_1 > 3)
		{
			Global_19486.f_1 = 7;
		}
		func_213();
		func_259();
		iLocal_249 = 0;
		bLocal_279 = false;
		return;
	}
	if (func_2(2, Global_19456, 0))
	{
		bLocal_279 = false;
		func_213();
		Global_19486.f_1 = 8;
		if (bLocal_291)
		{
			func_259();
			iLocal_276 = 0;
			iLocal_249 = 0;
		}
		else if (iLocal_292 == iLocal_288 && !Global_262145.f_4678)
		{
			func_259();
			iLocal_276 = 0;
			iLocal_249 = 0;
		}
		else if (func_212() && !Global_262145.f_4677)
		{
			func_194();
			iLocal_249 = 1;
		}
		else
		{
			func_259();
			iLocal_276 = 0;
			iLocal_249 = 0;
		}
		iLocal_293 = -1;
		func_193();
		return;
	}
	func_181();
	if (iLocal_332 == 0)
	{
		if (func_2(2, Global_19455, 0))
		{
			iLocal_332 = 1;
		}
	}
	else if (func_180(2, Global_19455) && !bLocal_279)
	{
		func_5();
	}
}

void func_5()
{
	func_213();
	if (func_156(iLocal_294[iLocal_288]))
	{
		func_155();
		switch (iLocal_294[iLocal_288])
		{
			case 28:
				iVar0 = -1;
				break;
			
			case 17:
				iVar0 = 2;
				break;
			
			case 6:
				if (Global_262145.f_9615)
				{
					iVar0 = 129;
				}
				else
				{
					iVar0 = 223;
				}
				iVar3 = func_154(223);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 9:
				if (Global_262145.f_9615)
				{
					iVar0 = 129;
				}
				else
				{
					iVar0 = 224;
				}
				iVar3 = func_154(224);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 8:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 232;
				}
				iVar3 = func_154(232);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 7:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 246;
				}
				iVar3 = func_154(246);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 5:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 251;
				}
				iVar3 = func_154(251);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 3:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 252;
				}
				iVar3 = func_154(252);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 4:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 253;
				}
				iVar3 = func_154(253);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 1:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 161;
				}
				iVar3 = func_154(161);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 11:
				if (Global_262145.f_9615)
				{
					iVar0 = 129;
				}
				else
				{
					iVar0 = 202;
				}
				iVar3 = func_154(202);
				if (iVar3 != -1)
				{
					iVar1 = func_152(iVar3);
					iVar2 = func_151(Global_262145.f_5032[iVar3][iVar1]);
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				break;
			
			case 10:
				if (Global_262145.f_9615)
				{
					iVar0 = 2;
				}
				else
				{
					iVar0 = 184;
				}
				iVar1 = func_145();
				iVar2 = func_151(func_144(iVar1));
				if (iVar2 != -1)
				{
					func_149(&(Global_794709.f_4[iVar2]));
					func_148(Global_794709.f_4[iVar2].f_71);
					func_147();
				}
				break;
			
			case 12:
				iVar0 = 2;
				iVar3 = func_143();
				iVar1 = func_152(iVar3);
				iVar2 = func_151(func_142(iVar1, iVar3));
				if (iVar2 != -1 && Global_262145.f_6446[iVar3] == 2)
				{
					func_149(&(Global_794709.f_4[iVar2]));
					func_148(Global_794709.f_4[iVar2].f_71);
					func_147();
					func_141();
				}
				break;
			
			case 15:
				func_140();
				func_138();
				func_137(0);
				iVar0 = 222;
				break;
			
			case 14:
				func_140();
				func_138();
				func_137(0);
				iVar0 = 228;
				break;
			
			case 18:
				iVar0 = 1;
				break;
			
			case 16:
				iVar0 = func_135(0);
				func_134();
				func_138();
				func_137(0);
				func_129(func_130(0));
				break;
			
			case 2:
				iVar0 = 233;
				func_128();
				func_138();
				func_137(0);
				iVar4 = func_130(1);
				if (iVar4 == 13)
				{
					iVar0 = 235;
				}
				func_129(iVar4);
				break;
			
			case 0:
				iVar0 = 158;
				func_127();
				func_138();
				func_137(0);
				break;
			
			case 19:
				iVar0 = 0;
				break;
			
			case 22:
				iVar0 = 7;
				break;
			
			case 23:
				iVar0 = 4;
				break;
			
			case 21:
				iVar0 = 10;
				break;
			
			case 13:
				iVar3 = func_125();
				if (iVar3 != -1 && func_124(iVar3))
				{
					iVar0 = 129;
					iVar1 = func_152(iVar3);
					iVar2 = func_151(func_142(iVar1, iVar3));
					if (iVar2 != -1)
					{
						func_149(&(Global_794709.f_4[iVar2]));
						func_148(Global_794709.f_4[iVar2].f_71);
						func_147();
					}
				}
				else
				{
					iVar0 = 129;
				}
				break;
			
			case 20:
				iVar0 = 156;
				break;
			
			case 24:
				iVar0 = 3;
				break;
			
			case 25:
				iVar0 = 8;
				break;
			
			case 26:
				switch (iLocal_324[iLocal_289])
				{
					case 0:
						iVar0 = 15;
						break;
					
					case 1:
						iVar0 = 14;
						break;
					
					case 2:
						iVar0 = 11;
						break;
					
					case 3:
						iVar0 = 13;
						break;
					
					case 4:
						iVar0 = 12;
						break;
				}
				break;
			
			case 27:
				iVar0 = 100;
				break;
		}
		func_121();
		if ((iLocal_294[iLocal_288] == 16 || iLocal_294[iLocal_288] == 2) && !Global_262145.f_4678)
		{
			if (iLocal_293 == 0 && bLocal_281)
			{
				func_120();
				func_117(0, 32);
			}
			else if (iLocal_293 == 2 || (iLocal_293 == 1 && bLocal_281 == 0))
			{
				func_116();
			}
		}
		else if (iLocal_293 == 0 && bLocal_281)
		{
			func_120();
			func_117(0, 32);
		}
		if (Global_262145.f_4677 && !func_115())
		{
			func_138();
			func_137(0);
		}
		else if (Global_262145.f_4676 == 0 && iLocal_282 == 0)
		{
			if ((iLocal_293 == 1 && bLocal_281) || (iLocal_293 == 0 && !bLocal_281))
			{
				func_138();
				func_137(0);
			}
		}
		if (iVar0 == -1)
		{
			func_114(12);
		}
		else if (func_297())
		{
			func_114(21);
		}
		else
		{
			func_114(11);
		}
		if (!func_297())
		{
			iVar5 = func_108(20);
			if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) > 0)
			{
				if (!NETWORKCASH::_0x7303E27CC6532080(iVar5, 0, 1, 0, &iVar6, -1, 0))
				{
					iVar5 = (iVar5 - iVar6);
				}
				if (iVar5 > 0)
				{
					Var8 = { func_107(PLAYER::PLAYER_ID()) };
					func_105(-iVar5, 1, 1, 0f);
					if (func_104())
					{
						func_92(-180141073, iVar5, &iVar7, 0, 1, 0);
						Global_4263954[iVar7].f_14 = { Var8 };
					}
					else
					{
						NETWORKCASH::NETWORK_SPENT_BUY_WANTEDLEVEL(iVar5, &Var8, 0, 1);
					}
				}
			}
			func_90();
			func_89();
			func_26(func_27(PLAYER::PLAYER_ID()));
			func_11(PLAYER::PLAYER_ID(), 0, 376832, 0);
			func_10(1, 1);
			Global_1590535[PLAYER::PLAYER_ID()].f_98 = 8;
		}
		if (iVar0 == 100)
		{
			func_9();
		}
		else if (iVar0 == -1)
		{
			func_8();
		}
		else
		{
			func_7(iVar0);
		}
		func_6();
		if (func_297())
		{
			func_215(0);
			func_295();
		}
	}
	iLocal_332 = 0;
}

void func_6()
{
	Global_2458936.f_1 = 0;
}

void func_7(int iParam0)
{
	Global_2450632.f_7 = iParam0;
}

void func_8()
{
	Global_2450632.f_7 = -1;
}

void func_9()
{
	Global_2450632.f_7 = 100;
}

void func_10(bool bParam0, bool bParam1)
{
	if (bParam0)
	{
		if (Global_2451426.f_2846.f_84 == 0)
		{
			if (!DLC2::GET_IS_LOADING_SCREEN_ACTIVE())
			{
				if (bParam1)
				{
					Global_2451426.f_2846.f_152 = 1;
					Global_2451426.f_2846.f_84 = 1;
				}
			}
		}
	}
	else if (Global_2451426.f_2846.f_84 == 1 || Global_2451426.f_2846.f_84 == 2)
	{
		Global_2451426.f_2846.f_84 = 3;
	}
}

void func_11(int iParam0, bool bParam1, int iParam2, int iParam3)
{
	if (bParam1)
	{
		if (SCRIPT::_GET_NO_LOADING_SCREEN())
		{
			SCRIPT::SET_NO_LOADING_SCREEN(0);
		}
	}
	if (func_25())
	{
		if (bParam1)
		{
			return;
		}
		else
		{
			iParam3 = 1;
		}
	}
	if (!NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		uVar0 = iParam2;
		PLAYER::SET_PLAYER_CONTROL(iParam0, bParam1, uVar0);
	}
	else
	{
		bVar1 = true;
		bVar2 = iParam2 & 2 != false;
		bVar3 = iParam2 & 4 != false;
		bVar4 = iParam2 & 8 != false;
		bVar5 = iParam2 & 16 != false;
		bVar6 = iParam2 & 32 != false;
		bVar7 = iParam2 & 64 != false;
		bVar8 = iParam2 & 128 != false;
		bVar9 = iParam2 & 256 != false;
		bVar10 = iParam2 & 512 != false;
		bVar11 = iParam2 & 1024 != false;
		bVar12 = iParam2 & 2048 != false;
		bVar13 = iParam2 & 4096 != false;
		bVar14 = iParam2 & 8192 != false;
		bVar15 = iParam2 & 16384 != false;
		bVar16 = iParam2 & 32768 != false;
		bVar17 = iParam2 & 65536 != false;
		bVar18 = iParam2 & 131072 != false;
		bVar19 = iParam2 & 262144 != false;
		bVar20 = iParam2 & 524288 != false;
		bVar21 = iParam2 & 1048576 != false;
		bVar22 = iParam2 & 2097152 != false;
		bVar23 = iParam2 & 4194304 != false;
		bVar24 = iParam2 & 8388608 != false;
		bVar25 = iParam2 & 16777216 != false;
		if (iParam2 & 33554432 != 0 || NETWORK::NETWORK_IS_ACTIVITY_SESSION())
		{
			bVar1 = false;
		}
		if (!func_23())
		{
			bVar26 = false;
			if (bParam1 == 1)
			{
				bVar26 = true;
			}
			if (bVar16 == 0 && !bVar21)
			{
				bVar26 = true;
			}
			if (bVar10 == 1)
			{
				bVar26 = true;
			}
			if (bVar26)
			{
				return;
			}
		}
		if (bVar18)
		{
		}
		if (NETWORK::NETWORK_IS_PLAYER_ACTIVE(iParam0) && (PLAYER::IS_PLAYER_PLAYING(iParam0) || iParam3 == 1))
		{
			if (iParam3 && ENTITY::IS_ENTITY_DEAD(PLAYER::GET_PLAYER_PED(iParam0), 0))
			{
				return;
			}
			iVar27 = PLAYER::GET_PLAYER_PED(iParam0);
			if (!bVar20)
			{
				if ((bVar19 && bParam1 == 0) && NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
				{
					NETWORK::FADE_OUT_LOCAL_PLAYER(1);
				}
				else if (bVar14 || (!func_20(PLAYER::PLAYER_ID(), 0) && !func_19()))
				{
					ENTITY::SET_ENTITY_VISIBLE(iVar27, !bVar14, 0);
				}
				if (!bVar14)
				{
					if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && !bVar19)
					{
						NETWORK::FADE_OUT_LOCAL_PLAYER(0);
					}
					Global_2425662[iParam0].f_244 = 0;
				}
			}
			if (bParam1)
			{
				if (bVar1)
				{
					func_16(0, 0, 0);
					if (bVar25)
					{
						STREAMING::CLEAR_FOCUS();
					}
				}
				if (!func_15(iVar27) && !ENTITY::IS_ENTITY_ATTACHED_TO_ANY_VEHICLE(iVar27))
				{
					if (!bVar22)
					{
						ENTITY::SET_ENTITY_COLLISION(iVar27, true, 0);
					}
				}
				if (!ENTITY::IS_ENTITY_ATTACHED(iVar27))
				{
					if (!bVar21)
					{
						ENTITY::FREEZE_ENTITY_POSITION(iVar27, false);
					}
					ENTITY::_SET_ENTITY_REGISTER(iVar27, 1);
				}
				else if (!bVar21)
				{
					ENTITY::FREEZE_ENTITY_POSITION(iVar27, false);
				}
				PED::SET_PED_CAN_BE_TARGETTED(iVar27, true);
				PLAYER::SET_PLAYER_INVINCIBLE(iParam0, 0);
				unk_0x6BC97F4F4BB3C04B(iParam0, 0);
				if (PED::HAS_PED_HEAD_BLEND_FINISHED(iVar27) && PED::_0x7350823473013C02(iVar27))
				{
					PED::_0x4668D80430D6C299(iVar27);
				}
				PED::SET_PED_CAN_RAGDOLL(iVar27, 1);
				if (PLAYER::IS_PLAYER_CONTROL_ON(PLAYER::PLAYER_ID()) == 0)
				{
					func_14();
					func_13();
				}
				if (PLAYER::IS_PLAYER_TELEPORT_ACTIVE())
				{
					if (!bVar23)
					{
					}
				}
				if (STREAMING::IS_NEW_LOAD_SCENE_ACTIVE())
				{
				}
				Global_2425662[iParam0].f_245 = 0;
				if (!bVar24)
				{
					bVar3 = true;
				}
				if (Global_2405072.f_2672)
				{
					Global_2405072.f_2672 = 0;
				}
			}
			else
			{
				if (!func_15(iVar27) && !ENTITY::IS_ENTITY_ATTACHED_TO_ANY_VEHICLE(iVar27))
				{
					if (!bVar22)
					{
						ENTITY::SET_ENTITY_COLLISION(iVar27, !bVar15, 0);
					}
					if (!ENTITY::IS_ENTITY_ATTACHED(iVar27))
					{
						if (!bVar21)
						{
							ENTITY::FREEZE_ENTITY_POSITION(iVar27, bVar16);
						}
						if (!bVar16)
						{
							ENTITY::_SET_ENTITY_REGISTER(iVar27, 1);
						}
					}
					if (func_12(Global_4456448.f_232883))
					{
						ENTITY::FREEZE_ENTITY_POSITION(iVar27, true);
					}
				}
				if (Global_1312854)
				{
					bVar10 = false;
				}
				if (bVar10)
				{
					PLAYER::SET_PLAYER_INVINCIBLE(iParam0, 0);
				}
				else
				{
					PLAYER::SET_PLAYER_INVINCIBLE(iParam0, 1);
				}
				PED::SET_PED_CAN_BE_TARGETTED(iVar27, bVar17);
				if (bVar3)
				{
					if (!PED::IS_PED_FATALLY_INJURED(iVar27) && !PED::IS_PED_IN_ANY_VEHICLE(iVar27, 0))
					{
						AI::CLEAR_PED_TASKS_IMMEDIATELY(iVar27);
					}
				}
			}
			iVar28 = 0;
			if (bVar2)
			{
				iVar28 |= 2;
			}
			if (bVar3)
			{
				iVar28 |= 4;
			}
			if (bVar4)
			{
				iVar28 |= 8;
			}
			if (bVar5)
			{
				iVar28 |= 16;
			}
			if (bVar6)
			{
				iVar28 |= 32;
			}
			if (bVar7)
			{
				iVar28 |= 64;
			}
			if (bVar8)
			{
				iVar28 |= 128;
			}
			if (bVar9)
			{
				iVar28 |= 256;
			}
			if (bVar10)
			{
				iVar28 |= 512;
			}
			if (bVar11)
			{
				iVar28 |= 1024;
			}
			if (bVar12)
			{
				iVar28 |= 2048;
			}
			if (bVar13)
			{
				iVar28 |= 4096;
			}
			PLAYER::SET_PLAYER_CONTROL(iParam0, bParam1, iVar28);
		}
	}
}

bool func_12(int iParam0)
{
	return iParam0 == 17;
}

void func_13()
{
	Global_2439138.f_1233 = 0;
	Global_2439138.f_1234 = 0;
	Global_2439138.f_1235 = { 9999.9f, 9999.9f, 9999.9f };
	Global_2439138.f_1240 = -1;
	Global_2439138.f_1241 = 0;
	Global_2405072.f_2683 = { Var0 };
}

void func_14()
{
	Global_2405072.f_694 = 0;
	Global_2405072.f_2726 = 0;
	Global_2405072.f_512 = 0;
	Global_2405072.f_600 = 0;
	Global_2425662[PLAYER::PLAYER_ID()].f_207 = 0;
	Global_2405072.f_2681 = 0;
}

int func_15(int iParam0)
{
	if (PED::IS_PED_IN_ANY_VEHICLE(iParam0, 1))
	{
		return 1;
	}
	else
	{
		iVar0 = AI::GET_SCRIPT_TASK_STATUS(iParam0, -1794415470);
		if (iVar0 == 0)
		{
			return 1;
		}
	}
	return 0;
}

void func_16(int iParam0, int iParam1, int iParam2)
{
	if (ENTITY::IS_ENTITY_DEAD(iParam1, 0))
	{
	}
	if (iParam0 == 1)
	{
		if (ENTITY::DOES_ENTITY_EXIST(iParam1))
		{
			if (PED::IS_PED_A_PLAYER(iParam1))
			{
				if (!NETWORK::_NETWORK_IS_PLAYER_EQUAL_TO_INDEX(PLAYER::PLAYER_ID(), NETWORK::_NETWORK_GET_PED_PLAYER(iParam1)))
				{
					iVar0 = 1;
				}
			}
		}
	}
	if (iVar0 == 0)
	{
		if (iParam2 == 1)
		{
			if (iParam0 == 1)
			{
				func_18();
			}
			else if (!GAMEPLAY::IS_BIT_SET(Global_2359302.f_67, 1))
			{
				if (GAMEPLAY::IS_BIT_SET(Global_2359302.f_67, 2))
				{
					iVar1 = 0;
					while (iVar1 < 8)
					{
						CAM::_0x2A2173E46DAECD12(iVar1, Global_2359302.f_58[iVar1]);
						iVar1++;
					}
				}
				GAMEPLAY::SET_BIT(&(Global_2359302.f_67), 1);
			}
		}
		if (func_20(PLAYER::PLAYER_ID(), 0))
		{
			NETWORK::_0x419594E137637120(iParam0, iParam1, 1);
		}
		else
		{
			NETWORK::NETWORK_SET_IN_SPECTATOR_MODE(iParam0, iParam1);
		}
		UI::KEY_HUD_COLOUR(iParam0, iParam1);
		func_17(-2008016205, iParam0);
	}
}

void func_17(int iParam0, int iParam1)
{
	iVar0 = iParam0;
	if (iVar0 != 0)
	{
		STATS::STAT_SET_BOOL(iVar0, iParam1, 1);
	}
}

void func_18()
{
	if (!NETWORK::NETWORK_IS_IN_SPECTATOR_MODE())
	{
		if (!GAMEPLAY::IS_BIT_SET(Global_2359302.f_67, 2))
		{
			iVar0 = 0;
			while (iVar0 < 8)
			{
				Global_2359302.f_58[iVar0] = CAM::_0xEE778F8C7E1142E2(iVar0);
				iVar0++;
			}
			GAMEPLAY::SET_BIT(&(Global_2359302.f_67), 2);
			GAMEPLAY::SET_BIT(&(Global_2359302.f_67), 0);
		}
	}
}

bool func_19()
{
	return GAMEPLAY::IS_BIT_SET(Global_2359302, 3);
}

bool func_20(int iParam0, int iParam1)
{
	if (iParam0 == PLAYER::PLAYER_ID())
	{
		bVar0 = func_21(-1, 0) == 8;
	}
	else
	{
		bVar0 = Global_1590535[iParam0].f_211 == 8;
	}
	if (iParam1 == 1)
	{
		if (NETWORK::NETWORK_IS_PLAYER_ACTIVE(iParam0))
		{
			bVar0 = PLAYER::GET_PLAYER_TEAM(iParam0) == 8;
		}
	}
	return bVar0;
}

int func_21(int iParam0, bool bParam1)
{
	iVar1 = iParam0;
	if (iVar1 == -1)
	{
		iVar1 = func_22();
	}
	if (Global_1312857[iVar1] == 1)
	{
		if (bParam1)
		{
		}
		iVar0 = 8;
	}
	else
	{
		iVar0 = Global_1312739[iVar1];
		if (bParam1)
		{
		}
	}
	return iVar0;
}

int func_22()
{
	return Global_1312745;
}

int func_23()
{
	if (func_24() == 0)
	{
		return 1;
	}
	return 0;
}

int func_24()
{
	return Global_1312467.f_18;
}

int func_25()
{
	if (Global_1590535[PLAYER::PLAYER_ID()].f_35 && !Global_2451426.f_2846.f_215 == -1)
	{
		return 1;
	}
	return 0;
}

void func_26(struct<3> Param0)
{
	Global_2460667 = { Param0 };
}

Vector3 func_27(int iParam0)
{
	if (UI::DOES_BLIP_EXIST(Global_2416079[iParam0]))
	{
		Var0 = { func_88(Global_2416079[iParam0]) };
	}
	else if (UI::DOES_BLIP_EXIST(Global_2416079.f_33[iParam0]) && !iParam0 == Global_2439138)
	{
		Var0 = { func_88(Global_2416079.f_33[iParam0]) };
	}
	if (SYSTEM::VMAG(Var0) > 0f)
	{
		return Var0;
	}
	return func_28(iParam0);
}

Vector3 func_28(int iParam0)
{
	if (Global_1590535[iParam0].f_274.f_28 > 0)
	{
		Var1 = { func_86(func_87(Global_1590535[PLAYER::PLAYER_ID()].f_274.f_28)) };
		if (SYSTEM::VMAG(Var1) > 0f)
		{
			return Var1;
		}
	}
	else
	{
		func_83(func_85(iParam0), &iVar0);
		if (iVar0 > 0)
		{
			Var1 = { func_86(func_87(iVar0)) };
			if (SYSTEM::VMAG(Var1) > 0f)
			{
				return Var1;
			}
		}
	}
	if (func_82(iParam0))
	{
		Var1 = { func_35(iParam0) };
		if (SYSTEM::VMAG(Var1) > 0f)
		{
			return Var1;
		}
	}
	if (func_33(iParam0))
	{
		iVar4 = func_31(iParam0);
		if (iVar4 > -1)
		{
			Var1 = { Global_1676377.f_488[iVar4] };
			if (SYSTEM::VMAG(Var1) > 0f)
			{
				return Var1;
			}
		}
	}
	if (func_30(iParam0))
	{
		Var1 = { func_29(iParam0) };
		if (SYSTEM::VMAG(Var1) > 0f)
		{
			return Var1;
		}
	}
	if (Global_2425662[iParam0].f_68.f_10)
	{
		if (SYSTEM::VMAG(Global_2425662[iParam0].f_68.f_11) > 0f)
		{
			return Global_2425662[iParam0].f_68.f_11;
		}
	}
	return func_85(iParam0);
}

Vector3 func_29(int iParam0)
{
	return Global_2425662[iParam0].f_68.f_7;
}

int func_30(int iParam0)
{
	if (Global_2425662[iParam0].f_68.f_6 != -1)
	{
		return 1;
	}
	return 0;
}

int func_31(int iParam0)
{
	if (iParam0 != func_32())
	{
		return Global_2425662[iParam0].f_310.f_13;
	}
	return -1;
}

int func_32()
{
	return -1;
}

int func_33(int iParam0)
{
	if (iParam0 != func_32() && (func_34(iParam0, 1, 1) || Global_2405072.f_2671))
	{
		bVar0 = GAMEPLAY::IS_BIT_SET(Global_2425662[iParam0].f_310, 1);
		return bVar0;
	}
	return 0;
}

int func_34(int iParam0, bool bParam1, bool bParam2)
{
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		if (NETWORK::NETWORK_IS_PLAYER_ACTIVE(iParam0))
		{
			if (bParam1)
			{
				if (!PLAYER::IS_PLAYER_PLAYING(iParam0))
				{
					return 0;
				}
			}
			if (bParam2)
			{
				if (!Global_2439138.f_3[iVar0])
				{
					return 0;
				}
			}
			return 1;
		}
	}
	return 0;
}

Vector3 func_35(int iParam0)
{
	if (iParam0 == func_32())
	{
	}
	if (func_81(iParam0))
	{
		iVar0 = func_80(iParam0);
		if (iVar0 != func_32())
		{
			if (!func_79(iVar0))
			{
				if (ENTITY::DOES_ENTITY_EXIST(Global_2439138.f_661[iVar0][1]))
				{
					return ENTITY::GET_ENTITY_COORDS(Global_2439138.f_661[iVar0][1], 0);
				}
				else
				{
					return Global_2425662[func_80(iParam0)].f_310.f_9;
				}
			}
			else
			{
				iVar1 = func_75(iVar0);
				if (!iVar1 == -1)
				{
					return Global_1676377.f_488[iVar1];
				}
			}
		}
	}
	else if (func_74(iParam0))
	{
		if (ENTITY::DOES_ENTITY_EXIST(Global_2537071.f_305))
		{
			return ENTITY::GET_ENTITY_COORDS(Global_2537071.f_305, 0);
		}
	}
	else if (func_73(iParam0) && !func_72(iParam0))
	{
		iVar2 = Global_2425662[iParam0].f_310.f_8;
		if (iVar2 != func_32())
		{
			iVar3 = func_75(iVar2);
			if (!iVar3 == -1)
			{
				return Global_1676377.f_488[iVar3];
			}
		}
	}
	else if (func_71(iParam0) && !func_70(iParam0))
	{
		if (func_82(iParam0) && func_69())
		{
			return Global_1676377.f_488[Global_2425662[iParam0].f_310.f_5];
		}
		iVar4 = Global_2425662[iParam0].f_310.f_8;
		if (iVar4 != func_32())
		{
			if (func_68(iVar4))
			{
				iVar5 = func_64(iVar4);
				if (iVar5 != -1)
				{
					return Global_1676377.f_488[iVar5];
				}
			}
		}
	}
	else if (func_63(iParam0))
	{
		iVar6 = func_62(iParam0);
		if (iVar6 != func_32())
		{
			if (!func_61(iVar6))
			{
				if (ENTITY::DOES_ENTITY_EXIST(Global_2439138.f_758[iVar6]))
				{
					return ENTITY::GET_ENTITY_COORDS(Global_2439138.f_758[iVar6], 0);
				}
				else
				{
					return Global_2425662[func_62(iParam0)].f_310.f_16;
				}
			}
			else
			{
				iVar7 = func_64(iVar6);
				if (!iVar7 == -1)
				{
					return Global_1676377.f_488[iVar7];
				}
			}
		}
	}
	else if (func_60(iParam0))
	{
		if (ENTITY::DOES_ENTITY_EXIST(Global_2537071.f_307))
		{
			return ENTITY::GET_ENTITY_COORDS(Global_2537071.f_307, 0);
		}
	}
	else if (func_59(iParam0) && !func_58(iParam0))
	{
		iVar8 = Global_2425662[iParam0].f_310.f_8;
		if (iVar8 != func_32())
		{
			if (func_57(iVar8))
			{
				iVar9 = func_53(iVar8);
				if (iVar9 != -1)
				{
					return func_52(iVar9);
				}
			}
		}
	}
	else if (func_51(iParam0) && !func_50(iParam0))
	{
		iVar10 = Global_2425662[iParam0].f_310.f_8;
		if (iVar10 != func_32())
		{
			if (func_49(iVar10))
			{
				iVar11 = func_45(iVar10);
				if (iVar11 != -1)
				{
					return Global_1676377.f_488[iVar11];
				}
			}
		}
	}
	else if (func_44(iParam0, 0))
	{
		iVar12 = func_43(iParam0);
		if (iVar12 != func_32())
		{
			if (!func_42(iVar12))
			{
				if (ENTITY::DOES_ENTITY_EXIST(Global_2439138.f_791[iVar12]))
				{
					return ENTITY::GET_ENTITY_COORDS(Global_2439138.f_791[iVar12], 0);
				}
				else
				{
					return Global_1628237[func_43(iParam0)].f_600;
				}
			}
			else
			{
				iVar13 = func_45(iVar12);
				if (!iVar13 == -1)
				{
					return Global_1676377.f_488[iVar13];
				}
			}
		}
	}
	else if (func_41(iParam0))
	{
		return -366.7f, -1909.6f, 20f;
	}
	if (func_59(iParam0))
	{
		return func_52(Global_2425662[iParam0].f_310.f_5);
	}
	if (func_36(iParam0))
	{
		return 965.8165f, 42.25042f, 122.1267f;
	}
	return Global_1676377.f_488[Global_2425662[iParam0].f_310.f_5];
}

int func_36(int iParam0)
{
	if ((func_40(iParam0) || func_39(iParam0)) || func_37(iParam0))
	{
		return 1;
	}
	return 0;
}

int func_37(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 15;
			}
		}
	}
	return 0;
}

int func_38(int iParam0)
{
	switch (iParam0)
	{
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 5:
		case 6:
		case 7:
		case 8:
		case 9:
		case 10:
		case 11:
		case 12:
		case 13:
		case 14:
		case 15:
		case 16:
		case 17:
		case 18:
		case 19:
		case 20:
		case 21:
			return 0;
			break;
		
		case 60:
		case 61:
		case 62:
		case 63:
		case 64:
		case 65:
		case 66:
		case 67:
		case 68:
		case 69:
			return 1;
			break;
		
		case 22:
		case 23:
		case 24:
		case 25:
		case 26:
		case 27:
		case 28:
		case 29:
		case 30:
		case 31:
		case 32:
		case 33:
		case 34:
		case 35:
		case 36:
		case 37:
		case 38:
		case 39:
		case 40:
		case 41:
			return 2;
			break;
		
		case 43:
		case 42:
		case 44:
		case 45:
		case 46:
		case 47:
		case 48:
		case 49:
		case 50:
		case 51:
		case 52:
		case 53:
		case 54:
		case 55:
		case 56:
		case 57:
		case 58:
		case 59:
		case 98:
		case 99:
		case 100:
		case 112:
		case 113:
		case 114:
		case 115:
		case 119:
		case 116:
		case 118:
		case 120:
		case 121:
		case 126:
		case 127:
		case 134:
		case 135:
		case 136:
		case 137:
		case 138:
		case 139:
		case 140:
		case 141:
		case 142:
		case 143:
		case 144:
			return 3;
			break;
		
		case 70:
		case 71:
		case 72:
		case 73:
		case 74:
		case 75:
		case 76:
		case 77:
		case 78:
		case 79:
		case 80:
			return 4;
			break;
		
		case 81:
			return 5;
			break;
		
		case 82:
			return 6;
			break;
		
		case 83:
		case 84:
		case 85:
		case 86:
		case 87:
			return 7;
			break;
		
		case 88:
			return 8;
			break;
		
		case 89:
		case 90:
		case 91:
		case 92:
		case 93:
		case 94:
		case 95:
		case 96:
		case 97:
			return 9;
			break;
		
		case 101:
			return 10;
			break;
		
		case 102:
		case 103:
		case 104:
		case 105:
		case 106:
		case 107:
		case 108:
		case 109:
		case 110:
		case 111:
			return 11;
			break;
		
		case 117:
			return 12;
			break;
		
		case 122:
			return 13;
			break;
		
		case 123:
			return 14;
			break;
		
		case 124:
			return 15;
			break;
		
		case 125:
			return 16;
			break;
		
		case 128:
		case 129:
		case 130:
		case 131:
		case 132:
		case 133:
			return 17;
			break;
		
		case 145:
			return 18;
			break;
	}
	return -1;
}

int func_39(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 16;
			}
		}
	}
	return 0;
}

int func_40(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 14;
			}
		}
	}
	return 0;
}

int func_41(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 13;
			}
		}
	}
	return 0;
}

int func_42(int iParam0)
{
	if (iParam0 != func_32())
	{
		return GAMEPLAY::IS_BIT_SET(Global_2425662[iParam0].f_310.f_3, 4);
	}
	return 0;
}

int func_43(int iParam0)
{
	if (iParam0 == func_32())
	{
		return iParam0;
	}
	return Global_2425662[iParam0].f_310.f_8;
}

int func_44(int iParam0, bool bParam1)
{
	if (bParam1)
	{
		if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), 0))
		{
			uVar0 = PED::GET_VEHICLE_PED_IS_IN(PLAYER::PLAYER_PED_ID(), 0);
			if (ENTITY::GET_ENTITY_MODEL(uVar0) == -1988428699)
			{
				return 1;
			}
		}
	}
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1 && Global_2425662[iParam0].f_310.f_8 != func_32())
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 12;
			}
		}
	}
	return 0;
}

int func_45(int iParam0)
{
	if (iParam0 != func_32())
	{
		iVar0 = func_48(iParam0);
		if (iVar0 != 0)
		{
			return func_46(iVar0);
		}
	}
	return -1;
}

int func_46(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 146)
	{
		if (func_38(iVar0) == 11)
		{
			if (func_47(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_47(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 1;
		
		case 103:
			return 2;
		
		case 104:
			return 3;
		
		case 105:
			return 4;
		
		case 106:
			return 5;
		
		case 107:
			return 6;
		
		case 108:
			return 7;
		
		case 109:
			return 8;
		
		case 110:
			return 9;
		
		case 111:
			return 10;
		
		default:
	}
	return 0;
}

int func_48(int iParam0)
{
	if (iParam0 != func_32())
	{
		return Global_1590535[iParam0].f_274.f_281;
	}
	return 0;
}

int func_49(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (Global_1590535[iParam0].f_274.f_281 != 0)
		{
			return 1;
		}
	}
	return 0;
}

int func_50(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_51(iParam0) && Global_2425662[iParam0].f_310.f_8 == iParam0)
		{
			return 1;
		}
	}
	return 0;
}

int func_51(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 11;
			}
		}
	}
	return 0;
}

Vector3 func_52(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 750.5f, -1322.3f, 26.2802f;
		
		case 103:
			return 331.9f, -974.9f, 30f;
		
		case 104:
			return -146f, -1270f, 28.3088f;
		
		case 105:
			return -17.7f, 225.7f, 106.7566f;
		
		case 106:
			return 894.4f, -2106.4f, 29.4768f;
		
		case 107:
			return -668f, -2431.5f, 12.9444f;
		
		case 108:
			return 213.4f, -3166.6f, 4.7903f;
		
		case 109:
			return 366.6f, 237.6f, 104.9f;
		
		case 110:
			return -1275.3f, -666.8f, 25.6332f;
		
		case 111:
			return -1188.8f, -1156.9f, 4.6582f;
		
		default:
	}
	return Global_1676377.f_488[iParam0];
}

int func_53(int iParam0)
{
	if (iParam0 != func_32())
	{
		iVar0 = func_56(iParam0);
		if (iVar0 != 0)
		{
			return func_54(iVar0);
		}
	}
	return -1;
}

int func_54(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 146)
	{
		if (func_38(iVar0) == 11)
		{
			if (func_55(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_55(int iParam0)
{
	switch (iParam0)
	{
		case 102:
			return 1;
		
		case 103:
			return 2;
		
		case 104:
			return 3;
		
		case 105:
			return 4;
		
		case 106:
			return 5;
		
		case 107:
			return 6;
		
		case 108:
			return 7;
		
		case 109:
			return 8;
		
		case 110:
			return 9;
		
		case 111:
			return 10;
		
		default:
	}
	return 0;
}

int func_56(int iParam0)
{
	if (iParam0 != func_32())
	{
		return Global_1590535[iParam0].f_274.f_322;
	}
	return 0;
}

int func_57(int iParam0)
{
	if (iParam0 != func_32())
	{
		return Global_1590535[iParam0].f_274.f_322 != 0;
	}
	return 0;
}

int func_58(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_59(iParam0) && Global_2425662[iParam0].f_310.f_8 == iParam0)
		{
			return 1;
		}
	}
	return 0;
}

int func_59(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 11;
			}
		}
	}
	return 0;
}

int func_60(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 10;
			}
		}
	}
	return 0;
}

int func_61(int iParam0)
{
	if (iParam0 != func_32())
	{
		return GAMEPLAY::IS_BIT_SET(Global_2425662[iParam0].f_310.f_2, 6);
	}
	return 0;
}

int func_62(int iParam0)
{
	if (iParam0 == func_32())
	{
		return iParam0;
	}
	return Global_2425662[iParam0].f_310.f_8;
}

int func_63(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1 && Global_2425662[iParam0].f_310.f_8 != func_32())
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 8;
			}
		}
	}
	return 0;
}

int func_64(int iParam0)
{
	if (iParam0 == func_32())
	{
		return -1;
	}
	iVar0 = func_67(iParam0);
	if (!iVar0 == 0)
	{
		return func_65(iVar0);
	}
	return -1;
}

int func_65(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 146)
	{
		if (func_38(iVar0) == 9)
		{
			if (func_66(iVar0) == iParam0)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

int func_66(int iParam0)
{
	switch (iParam0)
	{
		case 89:
			return 1;
			break;
		
		case 90:
			return 2;
			break;
		
		case 91:
			return 3;
			break;
		
		case 92:
			return 4;
			break;
		
		case 93:
			return 5;
			break;
		
		case 94:
			return 6;
			break;
		
		case 95:
			return 7;
			break;
		
		case 96:
			return 8;
			break;
		
		case 97:
			return 9;
			break;
	}
	return 0;
}

int func_67(int iParam0)
{
	if (iParam0 == func_32())
	{
		return 0;
	}
	return Global_1590535[iParam0].f_274.f_271;
}

int func_68(int iParam0)
{
	if (iParam0 != func_32())
	{
		return Global_1590535[iParam0].f_274.f_271 != 0;
	}
	return 0;
}

int func_69()
{
	if (GAMEPLAY::IS_BIT_SET(Global_1676377.f_2, 13) || Global_1676377.f_3015)
	{
		return 1;
	}
	return 0;
}

int func_70(int iParam0)
{
	if (iParam0 == func_32())
	{
		return 0;
	}
	if (func_71(iParam0) && Global_2425662[iParam0].f_310.f_8 == iParam0)
	{
		return 1;
	}
	return 0;
}

int func_71(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 9;
			}
		}
	}
	return 0;
}

int func_72(int iParam0)
{
	if (iParam0 == func_32())
	{
		return 0;
	}
	if (func_73(iParam0) && Global_2425662[iParam0].f_310.f_8 == iParam0)
	{
		return 1;
	}
	return 0;
}

int func_73(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 4;
			}
		}
	}
	return 0;
}

int func_74(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1)
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 6;
			}
		}
	}
	return 0;
}

int func_75(int iParam0)
{
	if (iParam0 == func_32())
	{
		return -1;
	}
	iVar0 = func_78(iParam0);
	if (!iVar0 == 0)
	{
		return func_76(iVar0);
	}
	return -1;
}

int func_76(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 146)
	{
		if (func_77(iVar0) == iParam0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

int func_77(int iParam0)
{
	switch (iParam0)
	{
		case 22:
			return 1;
			break;
		
		case 23:
			return 2;
			break;
		
		case 24:
			return 3;
			break;
		
		case 25:
			return 4;
			break;
		
		case 26:
			return 5;
			break;
		
		case 27:
			return 6;
			break;
		
		case 28:
			return 7;
			break;
		
		case 29:
			return 8;
			break;
		
		case 30:
			return 9;
			break;
		
		case 31:
			return 10;
			break;
		
		case 32:
			return 11;
			break;
		
		case 33:
			return 12;
			break;
		
		case 34:
			return 13;
			break;
		
		case 35:
			return 14;
			break;
		
		case 36:
			return 15;
			break;
		
		case 37:
			return 16;
			break;
		
		case 38:
			return 17;
			break;
		
		case 39:
			return 18;
			break;
		
		case 40:
			return 19;
			break;
		
		case 41:
			return 20;
			break;
		
		case 70:
			return 21;
			break;
		
		case 71:
			return 22;
			break;
		
		case 72:
			return 23;
			break;
		
		case 73:
			return 24;
			break;
		
		case 74:
			return 25;
			break;
		
		case 75:
			return 26;
			break;
		
		case 76:
			return 27;
			break;
		
		case 77:
			return 28;
			break;
		
		case 78:
			return 29;
			break;
		
		case 79:
			return 30;
			break;
		
		case 80:
			return 31;
			break;
	}
	return 0;
}

int func_78(int iParam0)
{
	if (iParam0 == func_32())
	{
		return 0;
	}
	return Global_1590535[iParam0].f_274.f_183[5];
}

int func_79(int iParam0)
{
	if (iParam0 != func_32())
	{
		return GAMEPLAY::IS_BIT_SET(Global_2425662[iParam0].f_310, 6);
	}
	return 0;
}

int func_80(int iParam0)
{
	if (iParam0 == func_32())
	{
		return iParam0;
	}
	return Global_2425662[iParam0].f_310.f_8;
}

int func_81(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			if (Global_2425662[iParam0].f_310.f_5 != -1 && Global_2425662[iParam0].f_310.f_8 != func_32())
			{
				return func_38(Global_2425662[iParam0].f_310.f_5) == 5;
			}
		}
	}
	return 0;
}

int func_82(int iParam0)
{
	if (iParam0 != func_32())
	{
		if (func_34(iParam0, 1, 1))
		{
			return Global_2425662[iParam0].f_310.f_5 != -1;
		}
		else if ((Global_1312877 && iParam0 == PLAYER::PLAYER_ID()) && func_34(iParam0, 1, 0))
		{
			return Global_2425662[iParam0].f_310.f_5 != -1;
		}
	}
	return 0;
}

void func_83(struct<3> Param0, var uParam3)
{
	iVar0 = 1;
	iVar1 = 7;
	while (iVar0 <= iVar1)
	{
		if (func_84(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 34;
	iVar1 = 43;
	while (iVar0 <= iVar1)
	{
		if (func_84(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 61;
	iVar1 = 65;
	while (iVar0 <= iVar1)
	{
		if (func_84(Param0, iVar0, 0))
		{
			*uParam3 = iVar0;
			return;
		}
		iVar0++;
	}
	iVar0 = 8;
	if (func_84(Param0, iVar0, 0))
	{
		if (Global_1590535[PLAYER::PLAYER_ID()].f_274.f_28 > 0)
		{
			*uParam3 = Global_1590535[PLAYER::PLAYER_ID()].f_274.f_28;
		}
		else if (Global_2404658.f_1 > 0)
		{
			*uParam3 = Global_2404658.f_1;
		}
		else
		{
			*uParam3 = iVar0;
		}
		return;
	}
	iVar0 = 17;
	if (func_84(Param0, iVar0, 0))
	{
		if (Global_1590535[PLAYER::PLAYER_ID()].f_274.f_28 > 0)
		{
			*uParam3 = Global_1590535[PLAYER::PLAYER_ID()].f_274.f_28;
		}
		else if (Global_2404658.f_1 > 0)
		{
			*uParam3 = Global_2404658.f_1;
		}
		else
		{
			*uParam3 = iVar0;
		}
		return;
	}
}

int func_84(struct<3> Param0, int iParam3, float fParam4)
{
	if (iParam3 != 999)
	{
		iVar21 = 0;
		while (iVar21 < 3)
		{
			if (Global_1049922[iParam3].f_146.f_57[iVar21].f_2 > Global_1049922[iParam3].f_146.f_57[iVar21].f_3.f_2)
			{
				Var1[iVar21] = { Global_1049922[iParam3].f_146.f_57[iVar21] };
				Var11[iVar21] = { Global_1049922[iParam3].f_146.f_57[iVar21].f_3 };
			}
			else
			{
				Var1[iVar21] = { Global_1049922[iParam3].f_146.f_57[iVar21].f_3 };
				Var11[iVar21] = { Global_1049922[iParam3].f_146.f_57[iVar21] };
			}
			Var1[iVar21].f_2 = (Var1[iVar21].f_2 + fParam4);
			Var11[iVar21].f_2 = (Var11[iVar21].f_2 - fParam4);
			iVar21++;
		}
		if (((Global_1049922[iParam3].f_146.f_57[0].f_6 != 0f && OBJECT::IS_POINT_IN_ANGLED_AREA(Param0, Var11[0], Var1[0], Global_1049922[iParam3].f_146.f_57[0].f_6, 0, 1)) || (Global_1049922[iParam3].f_146.f_57[1].f_6 != 0f && OBJECT::IS_POINT_IN_ANGLED_AREA(Param0, Var11[1], Var1[1], Global_1049922[iParam3].f_146.f_57[1].f_6, 0, 1))) || (Global_1049922[iParam3].f_146.f_57[2].f_6 != 0f && OBJECT::IS_POINT_IN_ANGLED_AREA(Param0, Var11[2], Var1[2], Global_1049922[iParam3].f_146.f_57[2].f_6, 0, 1)))
		{
			return 1;
		}
	}
	else
	{
		iVar0 = 1;
		while (iVar0 <= 7)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 34;
		while (iVar0 <= 43)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 61;
		while (iVar0 <= 65)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 73;
		while (iVar0 <= 76)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 77;
		while (iVar0 <= 82)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 83;
		while (iVar0 <= 85)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 87;
		while (iVar0 <= 90)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 91;
		while (iVar0 <= 102)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		iVar0 = 103;
		while (iVar0 <= 113)
		{
			if (func_84(Param0, iVar0, fParam4))
			{
				return 1;
			}
			iVar0++;
		}
		if (func_84(Param0, 8, fParam4))
		{
			return 1;
		}
		if (func_84(Param0, 17, fParam4))
		{
			return 1;
		}
	}
	return 0;
}

Vector3 func_85(int iParam0)
{
	return ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED(iParam0), 0);
}

Vector3 func_86(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			Var0 = { -773.4775f, 310.6321f, 84.6981f };
			break;
		
		case 2:
			Var0 = { -252.5713f, -949.9199f, 30.221f };
			break;
		
		case 3:
			Var0 = { -1443.094f, -544.7684f, 33.7424f };
			break;
		
		case 4:
			Var0 = { -913.85f, -455.1392f, 38.5999f };
			break;
		
		case 5:
			Var0 = { -47.3145f, -585.9766f, 36.9593f };
			break;
		
		case 6:
			Var0 = { -932.7474f, -383.9246f, 37.9613f };
			break;
		
		case 7:
			Var0 = { -619.1315f, 37.8841f, 42.5883f };
			break;
		
		case 8:
			Var0 = { 284.9614f, -159.9891f, 63.6221f };
			break;
		
		case 9:
			Var0 = { 2.8889f, 35.7762f, 70.5349f };
			break;
		
		case 10:
			Var0 = { 9.74f, 84.6906f, 77.3975f };
			break;
		
		case 11:
			Var0 = { -512.1465f, 111.2223f, 62.351f };
			break;
		
		case 12:
			Var0 = { -197.3405f, 88.1053f, 68.7422f };
			break;
		
		case 13:
			Var0 = { -628.3212f, 165.8364f, 60.1651f };
			break;
		
		case 14:
			Var0 = { -973.3757f, -1429.425f, 6.6791f };
			break;
		
		case 15:
			Var0 = { -829.1362f, -855.0104f, 18.6297f };
			break;
		
		case 16:
			Var0 = { -757.5739f, -755.5591f, 25.5697f };
			break;
		
		case 17:
			Var0 = { -45.1289f, -57.0925f, 62.2531f };
			break;
		
		case 18:
			Var0 = { -206.7293f, 184.142f, 79.3279f };
			break;
		
		case 19:
			Var0 = { -811.7045f, -984.1961f, 13.1538f };
			break;
		
		case 20:
			Var0 = { -664.0032f, -853.6744f, 23.4325f };
			break;
		
		case 21:
			Var0 = { -1534.025f, -324.5247f, 46.5237f };
			break;
		
		case 22:
			Var0 = { -1561.381f, -412.1974f, 41.389f };
			break;
		
		case 23:
			Var0 = { -1608.851f, -429.184f, 39.439f };
			break;
		
		case 24:
			Var0 = { 964.3583f, -1034.199f, 39.8303f };
			break;
		
		case 25:
			Var0 = { 894.2868f, -885.5679f, 26.1212f };
			break;
		
		case 26:
			Var0 = { 821.1741f, -924.1658f, 25.1998f };
			break;
		
		case 27:
			Var0 = { 759.7933f, -759.8209f, 25.759f };
			break;
		
		case 28:
			Var0 = { 844.7289f, -1164.1f, 24.2681f };
			break;
		
		case 29:
			Var0 = { 526.2521f, -1604.613f, 28.2625f };
			break;
		
		case 30:
			Var0 = { 572.0462f, -1570.736f, 27.4904f };
			break;
		
		case 31:
			Var0 = { 722.2852f, -1190.617f, 23.282f };
			break;
		
		case 32:
			Var0 = { 497.6212f, -1494.084f, 28.2893f };
			break;
		
		case 33:
			Var0 = { 480.3127f, -1549.937f, 28.2828f };
			break;
		
		case 34:
			Var0 = { -68.702f, 6426.148f, 30.439f };
			break;
		
		case 35:
			Var0 = { -247.4374f, 6240.294f, 30.4892f };
			break;
		
		case 36:
			Var0 = { 2554.165f, 4668.059f, 33.0233f };
			break;
		
		case 37:
			Var0 = { 2461.22f, 1589.255f, 32.0443f };
			break;
		
		case 38:
			Var0 = { -2203.335f, 4244.427f, 47.3305f };
			break;
		
		case 39:
			Var0 = { 218.0665f, 2601.817f, 44.7668f };
			break;
		
		case 40:
			Var0 = { 186.1719f, 2786.343f, 45.0144f };
			break;
		
		case 41:
			Var0 = { 642.0746f, 2791.729f, 40.9795f };
			break;
		
		case 42:
			Var0 = { -1130.938f, 2701.133f, 17.8004f };
			break;
		
		case 43:
			Var0 = { -10.944f, -1646.76f, 28.3125f };
			break;
		
		case 44:
			Var0 = { 1024.263f, -2398.404f, 29.1261f };
			break;
		
		case 45:
			Var0 = { 870.8577f, -2232.323f, 29.5508f };
			break;
		
		case 46:
			Var0 = { -663.8541f, -2380.389f, 12.9446f };
			break;
		
		case 47:
			Var0 = { -1088.616f, -2235.098f, 12.2182f };
			break;
		
		case 48:
			Var0 = { -342.5126f, -1468.675f, 29.6107f };
			break;
		
		case 49:
			Var0 = { -1241.54f, -259.8841f, 37.9445f };
			break;
		
		case 50:
			Var0 = { 899.8448f, -147.528f, 75.5674f };
			break;
		
		case 51:
			Var0 = { -1405.411f, 526.8572f, 122.8361f };
			break;
		
		case 52:
			Var0 = { 1341.552f, -1578.037f, 53.4443f };
			break;
		
		case 53:
			Var0 = { -105.7029f, 6528.539f, 29.1719f };
			break;
		
		case 54:
			Var0 = { -302.3985f, 6327.434f, 31.8918f };
			break;
		
		case 55:
			Var0 = { -15.258f, 6557.378f, 32.2454f };
			break;
		
		case 56:
			Var0 = { 1899.673f, 3773.178f, 31.8829f };
			break;
		
		case 57:
			Var0 = { 1662.121f, 4776.317f, 41.0075f };
			break;
		
		case 58:
			Var0 = { -178.2278f, 490.886f, 134.0466f };
			break;
		
		case 59:
			Var0 = { 339.5743f, 439.7083f, 145.5896f };
			break;
		
		case 60:
			Var0 = { -764.6163f, 618.3144f, 137.5536f };
			break;
		
		case 61:
			Var0 = { -679.5461f, 592.5162f, 139.693f };
			break;
		
		case 62:
			Var0 = { 124.4571f, 551.8877f, 181.658f };
			break;
		
		case 63:
			Var0 = { -563.7349f, 689.099f, 151.6596f };
			break;
		
		case 64:
			Var0 = { -743.0927f, 590.0371f, 140.9221f };
			break;
		
		case 65:
			Var0 = { -861.252f, 684.8923f, 146.2643f };
			break;
		
		case 66:
			Var0 = { -1292.456f, 440.9454f, 93.7572f };
			break;
		
		case 67:
			Var0 = { 369.5891f, 417.4813f, 136.8344f };
			break;
		
		case 68:
			Var0 = { -1581.501f, -558.2578f, 33.9528f };
			break;
		
		case 69:
			Var0 = { -1379.546f, -499.1783f, 32.1574f };
			break;
		
		case 70:
			Var0 = { -117.5296f, -605.7157f, 35.2857f };
			break;
		
		case 71:
			Var0 = { -67.0943f, -802.4415f, 43.2273f };
			break;
		
		case 72:
			Var0 = { 254.1892f, -1809.183f, 26.1805f };
			break;
		
		case 73:
			Var0 = { -1472.278f, -920.0677f, 8.9683f };
			break;
		
		case 74:
			Var0 = { 38.9478f, -1026.629f, 28.6354f };
			break;
		
		case 75:
			Var0 = { 46.903f, 2789.825f, 57.1124f };
			break;
		
		case 76:
			Var0 = { -342.3246f, 6065.316f, 30.4895f };
			break;
		
		case 77:
			Var0 = { 1737.878f, 3709.088f, 33.1348f };
			break;
		
		case 78:
			Var0 = { 939.7161f, -1459.204f, 30.467f };
			break;
		
		case 79:
			Var0 = { 189.7624f, 309.7488f, 104.4714f };
			break;
		
		case 80:
			Var0 = { -21.9593f, -191.3618f, 51.5599f };
			break;
		
		case 81:
			Var0 = { 2472.22f, 4110.493f, 36.9629f };
			break;
		
		case 82:
			Var0 = { -39.3286f, 6420.603f, 30.7017f };
			break;
		
		case 83:
			Var0 = { -1134.762f, -1568.848f, 3.4077f };
			break;
	}
	return Var0;
}

int func_87(int iParam0)
{
	switch (iParam0)
	{
		case 1:
		case 2:
		case 3:
		case 4:
		case 61:
		case 83:
		case 84:
		case 85:
			return 1;
			break;
		
		case 5:
		case 6:
			return 2;
			break;
		
		case 7:
		case 34:
		case 62:
			return 3;
			break;
		
		case 35:
		case 36:
		case 37:
			return 4;
			break;
		
		case 38:
		case 39:
		case 65:
			return 5;
			break;
		
		case 40:
		case 41:
		case 63:
			return 6;
			break;
		
		case 42:
		case 43:
		case 64:
			return 7;
			break;
		
		case 8:
			return 8;
			break;
		
		case 9:
			return 9;
			break;
		
		case 10:
			return 10;
			break;
		
		case 11:
			return 11;
			break;
		
		case 12:
			return 12;
			break;
		
		case 13:
			return 13;
			break;
		
		case 14:
			return 14;
			break;
		
		case 15:
			return 15;
			break;
		
		case 16:
			return 16;
			break;
		
		case 17:
			return 17;
			break;
		
		case 18:
			return 18;
			break;
		
		case 19:
			return 19;
			break;
		
		case 20:
			return 20;
			break;
		
		case 21:
			return 21;
			break;
		
		case 22:
			return 22;
			break;
		
		case 23:
			return 23;
			break;
		
		case 24:
			return 24;
			break;
		
		case 25:
			return 25;
			break;
		
		case 26:
			return 26;
			break;
		
		case 27:
			return 27;
			break;
		
		case 28:
			return 28;
			break;
		
		case 29:
			return 29;
			break;
		
		case 30:
			return 30;
			break;
		
		case 31:
			return 31;
			break;
		
		case 32:
			return 32;
			break;
		
		case 33:
			return 33;
			break;
		
		case 44:
			return 34;
			break;
		
		case 45:
			return 35;
			break;
		
		case 46:
			return 36;
			break;
		
		case 47:
			return 37;
			break;
		
		case 48:
			return 38;
			break;
		
		case 49:
			return 39;
			break;
		
		case 50:
			return 40;
			break;
		
		case 51:
			return 41;
			break;
		
		case 52:
			return 42;
			break;
		
		case 53:
			return 43;
			break;
		
		case 54:
			return 44;
			break;
		
		case 55:
			return 45;
			break;
		
		case 56:
			return 46;
			break;
		
		case 57:
			return 47;
			break;
		
		case 58:
			return 48;
			break;
		
		case 59:
			return 49;
			break;
		
		case 60:
			return 50;
			break;
		
		case 66:
			return 51;
			break;
		
		case 67:
			return 52;
			break;
		
		case 68:
			return 53;
			break;
		
		case 69:
			return 54;
			break;
		
		case 70:
			return 55;
			break;
		
		case 71:
			return 56;
			break;
		
		case 72:
			return 57;
			break;
		
		case 73:
			return 58;
			break;
		
		case 74:
			return 59;
			break;
		
		case 75:
			return 60;
			break;
		
		case 76:
			return 61;
			break;
		
		case 77:
			return 62;
			break;
		
		case 78:
			return 63;
			break;
		
		case 79:
			return 64;
			break;
		
		case 80:
			return 65;
			break;
		
		case 81:
			return 66;
			break;
		
		case 82:
			return 67;
			break;
		
		case 87:
		case 103:
		case 104:
		case 105:
			return 68;
			break;
		
		case 88:
		case 106:
		case 107:
		case 108:
			return 69;
			break;
		
		case 89:
		case 109:
		case 110:
		case 111:
			return 70;
			break;
		
		case 90:
		case 112:
		case 113:
		case 114:
			return 71;
			break;
		
		case 91:
			return 72;
			break;
		
		case 92:
			return 73;
			break;
		
		case 93:
			return 74;
			break;
		
		case 94:
			return 75;
			break;
		
		case 95:
			return 76;
			break;
		
		case 96:
			return 77;
			break;
		
		case 97:
			return 78;
			break;
		
		case 98:
			return 79;
			break;
		
		case 99:
			return 80;
			break;
		
		case 100:
			return 81;
			break;
		
		case 101:
			return 82;
			break;
		
		case 102:
			return 83;
			break;
	}
	return 0;
}

Vector3 func_88(var uParam0)
{
	switch (UI::GET_BLIP_INFO_ID_TYPE(uParam0))
	{
		case 1:
		case 2:
		case 3:
			uVar0 = UI::GET_BLIP_INFO_ID_ENTITY_INDEX(uParam0);
			if (ENTITY::DOES_ENTITY_EXIST(uVar0))
			{
				return ENTITY::GET_ENTITY_COORDS(iVar0, 0);
			}
			break;
	}
	return UI::GET_BLIP_COORDS(uParam0);
}

void func_89()
{
	GAMEPLAY::SET_BIT(&Global_2450632, 7);
}

void func_90()
{
	GAMEPLAY::SET_BIT(&Global_2450632, 5);
	func_91();
}

void func_91()
{
	GAMEPLAY::SET_BIT(&Global_2450632, 8);
}

void func_92(int iParam0, int iParam1, var uParam2, bool bParam3, bool bParam4, bool bParam5)
{
	if (!func_104())
	{
		return;
	}
	iVar0 = 1;
	if (bParam4)
	{
		iVar0 = 4;
	}
	else if (bParam3)
	{
		iVar0 = 2;
	}
	else if (bParam5)
	{
		iVar0 = 8;
	}
	switch (iParam0)
	{
		case -1645229221:
		case -585718395:
		case -1359375127:
		case 454359403:
		case -982394051:
		case 1643659499:
		case -2072289654:
		case 650665123:
		case 1654961868:
		case -876847842:
		case 68030260:
		case -2122299283:
		case 366672791:
		case 283351220:
		case 291576838:
		case 1182673174:
		case -516219046:
		case 1036455748:
		case 277665518:
		case 2043854386:
		case 1839532116:
		case 1022400740:
		case 1940862352:
		case -1389227906:
		case 711665950:
		case 1704445500:
		case 1515774909:
		case 1173654533:
		case -899802304:
		case -663944335:
		case 1208553146:
		case -613221010:
		case -671062876:
		case -407201236:
		case -754024203:
		case -1885444887:
		case 1931729587:
		case 1064954035:
		case -180141073:
		case 2131324797:
		case 1612072658:
		case -517447402:
		case 1948102096:
		case 1108628223:
		case -1834046564:
		case -1239008812:
		case -222363842:
		case -1276678868:
		case 1564537328:
		case -96593501:
		case 742499889:
		case 2050093329:
		case -1752488069:
		case 634375935:
		case 1850983186:
		case 763367758:
		case 1941570214:
		case 665109499:
		case -1330755006:
		case 1976384368:
		case 268924934:
		case 1869490922:
		case -336803850:
		case -1412692765:
		case 618167454:
		case 980623936:
		case 437291904:
		case -135813048:
		case -930104477:
		case -1420909320:
		case -1733398043:
		case -1892760262:
		case -1545737048:
		case 2039302543:
		case 402505853:
		case -1143510182:
		case 1678112166:
		case 837955913:
		case -1532467144:
		case 1815541181:
			if (iParam1 > 0 || Global_262145.f_27584)
			{
				func_93(uParam2, -1135378931, 537254313, 1474183246, iParam0, iParam1, iVar0, 7);
			}
			break;
		
		case -31156877:
		case -1027218631:
		case -1398318418:
		case 1652884147:
		case -57868256:
		case -1216489292:
		case -46622315:
		case -352356931:
		case -990286235:
		case 563463121:
		case 1734805203:
		case 941287179:
		case -1186079845:
		case -1985150258:
		case -1127021384:
		case -109201286:
		case 312105838:
		case 1982688246:
		case -661030418:
		case 1301046174:
		case -1586170317:
		case 393059668:
		case 23796958:
		case -1077156170:
		case 1780666425:
		case -2043695058:
		case -1922554349:
		case 1287308202:
		case 691372038:
		case 1480707108:
		case 1512499951:
		case 562283735:
		case -154732333:
		case -1362660491:
		case 645708827:
		case 767907967:
		case -1970151306:
		case 718859568:
		case -1955564771:
		case 892388724:
		case -1426920838:
		case 1349151477:
		case 1620609399:
		case 1961641934:
		case 210955503:
		case -59668082:
		case 1736933716:
		case -1468524125:
		case 111573502:
		case 1525644423:
		case 968073639:
		case 1577781788:
		case -934465332:
		case -1194253122:
		case -212607085:
		case -815546555:
		case 1048226110:
		case 569170531:
		case -856006867:
		case 848090538:
		case -47546905:
		case -293060240:
		case 463142405:
		case 1550217370:
		case -101307780:
		case -664597565:
		case 599804707:
		case -327918414:
		case 550898518:
		case 835976347:
		case 1347433368:
		case 1052472386:
		case -2130199671:
		case -1227654538:
		case 1864522104:
		case 215608230:
		case -876012764:
		case -722894325:
		case 1407278493:
		case -1579394494:
		case -27443911:
		case 1179783540:
		case 923419301:
		case -308826175:
		case 603298940:
		case -12619854:
		case -311112675:
		case 870439158:
		case -974288740:
		case -4138654:
		case -1180954122:
		case -1918051016:
		case 844330594:
		case 1934825517:
		case 1852024236:
		case 2099238988:
		case 1952643559:
		case -1172900789:
		case -2015399333:
		case -1574795641:
		case -961034881:
		case 1135468152:
		case 1265272476:
		case -634726636:
		case 696556762:
		case 403506509:
		case -883876414:
		case -1064150715:
		case -1387253055:
		case -716963152:
		case 1138089938:
		case -561012053:
			func_93(uParam2, -1135378931, 1445302971, 1474183246, iParam0, iParam1, iVar0, 7);
			break;
	}
	switch (iParam0)
	{
		case -1342064661:
		case -1693570755:
		case 1868043300:
		case -697248883:
		case -1295545795:
		case 914079366:
		case 395122350:
		case -331981076:
		case 1671535231:
			if (iParam1 > 0 || Global_262145.f_27584)
			{
				func_93(uParam2, -1135378931, 537254313, 1474183246, iParam0, iParam1, iVar0, 7);
			}
			break;
		
		case 1240683675:
		case 1241904665:
		case -1100963799:
		case -494565059:
		case 827308208:
		case -1857685192:
		case 1698417709:
		case 1057653594:
		case 1810506918:
		case 451427308:
		case 824622151:
		case 1253978276:
		case -1576080766:
		case 1508411869:
		case 1428501742:
		case -1918967151:
		case 1261538664:
		case 1180397655:
		case 1414674366:
		case 261460130:
		case -2027658376:
		case -2017925037:
		case 1668610896:
		case -2032529561:
		case -1224479447:
		case -319306689:
		case -466527264:
		case 1925965142:
		case 592152676:
		case 2035612943:
		case 1568659720:
		case 1220095570:
		case 2050320631:
		case 592672421:
		case 1775876058:
		case -842062976:
			func_93(uParam2, -1135378931, 1445302971, 1474183246, iParam0, iParam1, iVar0, 7);
			break;
	}
}

int func_93(var uParam0, int iParam1, int iParam2, int iParam3, int iParam4, int iParam5, int iParam6, int iParam7)
{
	bVar0 = false;
	if (!func_104())
	{
		bVar0 = true;
	}
	iVar1 = 1;
	if (!bVar0)
	{
		if (!UNK3::_0xB24F0944DA203D9E(func_22()) || UNK3::_0x810E8431C0614BF9())
		{
			Global_4264536 = 1;
			return 0;
		}
		if (Global_2461839)
		{
			if (iParam3 == 1067618600 || iParam3 == -1303831698)
			{
				Global_4264537 = 1;
				return 0;
			}
		}
	}
	bVar3 = false;
	iVar2 = 0;
	while (iVar2 < 5)
	{
		if (Global_4263954[iVar2].f_66.f_2 == 0)
		{
			bVar3 = true;
		}
		iVar2++;
	}
	if (!bVar3)
	{
		return 0;
	}
	*uParam0 = 5;
	iVar4 = 2147483647;
	if ((bVar0 || iVar1) || UNK3::_NETWORK_SHOP_BEGIN_SERVICE(&iVar4, iParam3, iParam4, iParam2, iParam5, iParam6))
	{
		if ((bVar0 || iVar1) || UNK3::_NETWORK_SHOP_CHECKOUT_START(iVar4))
		{
			*uParam0 = func_100(iVar4, iParam1, iParam4, iParam2, iParam3, iParam5, 0, iParam6, iParam7, 1, 1);
			if (iVar1 && !bVar0)
			{
			}
			if (bVar0)
			{
				if (*uParam0 != -1)
				{
					Global_4263954[*uParam0].f_66.f_8 = 1;
					Global_4263954[*uParam0].f_66.f_12 = 1;
				}
			}
			Global_4264521 = 1;
			return 1;
		}
	}
	else
	{
		if (iParam7 & 2 != 0)
		{
			Global_4264535 = 1;
			Global_4264538 = iParam4;
			Global_4264540 = iParam3;
			Global_4264541 = 1;
			Global_4264539 = iParam5;
		}
		if (iParam7 & 8 != 0)
		{
			Global_4264538 = iParam4;
			Global_4264540 = iParam3;
			Global_4264541 = 1;
			Global_4264539 = iParam5;
		}
		bVar5 = false;
		if (bVar5)
		{
			func_99(1, iParam4);
			Global_4264535 = 0;
		}
		if (iParam7 & 4 != 0)
		{
			func_94(-1, iParam4, iParam6, iParam5, -1);
		}
	}
	return 0;
}

void func_94(int iParam0, int iParam1, var uParam2, var uParam3, int iParam4)
{
	switch (iParam1)
	{
		case 1704445500:
			GAMEPLAY::SET_BIT(&(Global_2425662[PLAYER::PLAYER_ID()].f_122.f_71), 0);
			break;
	}
	if (iParam0 != -1)
	{
		func_95(iParam0);
	}
}

void func_95(int iParam0)
{
	bVar0 = false;
	if (!func_104())
	{
		bVar0 = true;
	}
	if (iParam0 != -1)
	{
		if (func_98(iParam0))
		{
			if (!bVar0)
			{
				unk_0xFA336E7F40C0A0D0();
			}
		}
		else if (!bVar0)
		{
			UNK3::_NETWORK_SHOP_END_SERVICE(Global_4263954[iParam0].f_66);
		}
		func_96(&(Global_4263954[iParam0]));
	}
}

void func_96(var uParam0)
{
	uParam0->f_66 = 0;
	uParam0->f_66 = 2147483647;
	uParam0->f_66.f_1 = 0;
	uParam0->f_66.f_2 = 0;
	uParam0->f_66.f_3 = -1593119440;
	uParam0->f_66.f_4 = -2085313189;
	uParam0->f_66.f_5 = 0;
	uParam0->f_66.f_6 = 1227573907;
	uParam0->f_66.f_7 = -1161833819;
	*uParam0 = 0;
	uParam0->f_1 = 0;
	uParam0->f_13 = 0;
	uParam0->f_2 = 0;
	func_97(&(uParam0->f_14));
	func_97(&(uParam0->f_14.f_13));
	StringCopy(&(uParam0->f_14.f_26), "", 32);
	StringCopy(&(uParam0->f_14.f_34), "", 24);
	StringCopy(&(uParam0->f_14.f_40), "", 16);
	StringCopy(&(uParam0->f_14.f_44), "", 32);
	uParam0->f_3 = 0;
	uParam0->f_4 = 0;
	uParam0->f_5 = 0;
	uParam0->f_6 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = 0;
	uParam0->f_66.f_8 = 0;
	uParam0->f_66.f_9 = 0;
	uParam0->f_66.f_10 = 0;
	uParam0->f_66.f_11 = 0;
	uParam0->f_66.f_13 = 0;
	uParam0->f_66.f_12 = 0;
	uParam0->f_66.f_14 = 0;
	uParam0->f_66.f_15 = 0;
	uParam0->f_66.f_16 = 0;
	uParam0->f_66.f_18 = 0;
}

void func_97(var uParam0)
{
	*uParam0 = 0;
	uParam0->f_1 = 0;
	uParam0->f_2 = 0;
	uParam0->f_3 = 0;
	uParam0->f_4 = 0;
	uParam0->f_5 = 0;
	uParam0->f_6 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = 0;
	uParam0->f_9 = 0;
	uParam0->f_10 = 0;
	uParam0->f_11 = 0;
	uParam0->f_12 = 0;
}

int func_98(int iParam0)
{
	if (iParam0 >= 0 && iParam0 < 5)
	{
		return Global_4263954[iParam0].f_66.f_5 == 1;
	}
	return 0;
}

void func_99(int iParam0, var uParam1)
{
	Global_2463019 = uParam1;
	Global_2463018 = iParam0;
}

int func_100(int iParam0, int iParam1, var uParam2, var uParam3, int iParam4, var uParam5, bool bParam6, var uParam7, var uParam8, int iParam9, int iParam10)
{
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 5)
	{
		if (Global_4263954[iVar0].f_66.f_2 == 0)
		{
			if (!func_104())
			{
				iParam0 = iVar0 + 900;
			}
			Global_4263954[iVar0].f_66.f_2 = 1;
			Global_4263954[iVar0].f_66.f_1 = uParam5;
			Global_4263954[iVar0].f_66.f_3 = iParam1;
			Global_4263954[iVar0].f_66.f_4 = uParam2;
			Global_4263954[iVar0].f_66.f_7 = uParam3;
			Global_4263954[iVar0].f_66.f_5 = 0;
			Global_4263954[iVar0].f_66 = iParam0;
			Global_4263954[iVar0].f_66.f_6 = iParam4;
			Global_4263954[iVar0].f_66.f_11 = uParam8;
			Global_4263954[iVar0].f_66.f_10 = uParam7;
			Global_4263954[iVar0].f_66.f_13 = iParam9;
			Global_4263954[iVar0].f_66.f_12 = 0;
			Global_4263954[iVar0].f_66.f_14 = GAMEPLAY::GET_FRAME_COUNT();
			Global_4263954[iVar0].f_66.f_18 = 0;
			Global_4264521 = 0;
			if (bParam6)
			{
				Global_4263954[iVar0].f_66.f_5 = 1;
			}
			if (iParam1 == -1135378931 && iParam10)
			{
				func_101(Global_4263954[iVar0], iVar0);
			}
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

void func_101(struct<67> Param0, var uParam67, var uParam68, var uParam69, var uParam70, var uParam71, var uParam72, var uParam73, var uParam74, var uParam75, var uParam76, var uParam77, var uParam78, var uParam79, var uParam80, var uParam81, var uParam82, var uParam83, var uParam84, int iParam85)
{
	if (iParam85 < 0)
	{
		return;
	}
	Var0.f_2 = 2147483647;
	Var0.x = 285918879;
	Var0.y = PLAYER::PLAYER_ID();
	Var0.f_2 = { Param0.f_66 };
	Var0.f_2.f_33 = iParam85;
	iVar36 = func_103(Var0.y);
	if ((Global_262145.f_23568 && !Global_262145.f_23569) && !Global_262145.f_23570)
	{
		return;
	}
	if (!iVar36 == 0)
	{
		func_102();
		SCRIPT::TRIGGER_SCRIPT_EVENT(1, &Var0, 36, iVar36);
	}
}

void func_102()
{
	SCRIPT::REQUEST_SCRIPT("AM_ARENA_SHP");
}

var func_103(int iParam0)
{
	if (iParam0 != -1)
	{
		GAMEPLAY::SET_BIT(&uVar0, iParam0);
	}
	return uVar0;
}

int func_104()
{
	if (GAMEPLAY::IS_PC_VERSION())
	{
		return unk_0x7D2708796355B20B();
	}
	return 0;
}

void func_105(int iParam0, int iParam1, int iParam2, float fParam3)
{
	if (iParam1 < 1)
	{
		iParam1 = 1;
	}
	iVar0 = (iParam0 * iParam1);
	fParam3 = 0f;
	if (iVar0 > 0)
	{
		fVar2 = (100f - fParam3);
		iVar1 = SYSTEM::FLOOR((IntToFloat(iVar0) * (fVar2 / 100f)));
	}
	else
	{
		iVar1 = iVar0;
	}
	Global_1590535[PLAYER::PLAYER_ID()].f_211.f_4 = iVar1;
	Global_1590535[PLAYER::PLAYER_ID()].f_211.f_3 = (Global_1590535[PLAYER::PLAYER_ID()].f_211.f_3 + iVar1);
	if (iParam2 == 1)
	{
		func_106(iVar1, 0);
	}
}

void func_106(int iParam0, bool bParam1)
{
	if (bParam1)
	{
	}
	iParam0 = iParam0;
}

struct<13> func_107(int iParam0)
{
	NETWORK::NETWORK_HANDLE_FROM_PLAYER(iParam0, &Var0, 13);
	return Var0;
}

int func_108(int iParam0)
{
	iVar0 = 0;
	if (func_113(iParam0) >= 0)
	{
		iVar0 = func_113(iParam0);
	}
	else
	{
		iVar0 = func_109(iParam0);
	}
	return iVar0;
}

int func_109(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return 1000;
			break;
		
		case 10:
			return 5000;
			break;
		
		case 11:
			return 8000;
			break;
		
		case 8:
			return 1000;
			break;
		
		case 0:
			return 500;
			break;
		
		case 9:
			return 250;
			break;
		
		case 13:
			return 1000;
			break;
		
		case 12:
			return 7500;
			break;
		
		case 2:
			return 1000;
			break;
		
		case 14:
			return 500;
			break;
		
		case 20:
			if (func_110())
			{
				return 0;
			}
			if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 1)
			{
				return 200;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 2)
			{
				return 400;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 3)
			{
				return 600;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 4)
			{
				return 800;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 5)
			{
				return 1000;
			}
			break;
		
		case 6:
			return 500;
			break;
		
		case 22:
			return 200;
			break;
		
		case 23:
			return 400;
			break;
		
		case 24:
			return 700;
			break;
		
		case 25:
			return 100;
			break;
		
		case 26:
			return 1000;
			break;
		
		case 57:
			return 700;
			break;
		
		case 35:
			return 5000;
			break;
		
		case 15:
			return 0;
			break;
		
		case 17:
			return 0;
			break;
		
		case 18:
			return 0;
			break;
		
		case 19:
			return 0;
			break;
		
		case 21:
			return 0;
			break;
		
		case 36:
			return 0;
			break;
		
		case 39:
			return 200;
			break;
		
		case 40:
			return 1000;
		
		case 41:
			return 750;
		
		case 42:
			return 0;
	}
	return 0;
}

bool func_110()
{
	return GAMEPLAY::IS_BIT_SET(func_111(6424, -1, 0), 19);
}

int func_111(int iParam0, int iParam1, int iParam2)
{
	if (iParam0 != 11511)
	{
		if (iParam2 == 0)
		{
		}
		uVar0 = Global_2548434[iParam0][func_112(iParam1)];
		if (STATS::STAT_GET_INT(uVar0, &uVar1, -1))
		{
			return uVar1;
		}
	}
	return 0;
}

int func_112(int iParam0)
{
	iVar0 = iParam0;
	if (iVar0 == -1)
	{
		iVar1 = func_22();
		if (iVar1 > -1)
		{
			Global_2548146 = 0;
			iVar0 = iVar1;
		}
		else
		{
			iVar0 = 0;
			Global_2548146 = 1;
		}
	}
	return iVar0;
}

int func_113(int iParam0)
{
	switch (iParam0)
	{
		case 1:
			return Global_262145.f_6645;
			break;
		
		case 10:
			return Global_262145.f_4118;
			break;
		
		case 11:
			return Global_262145.f_4119;
			break;
		
		case 8:
			return Global_262145.f_4116;
			break;
		
		case 0:
			return Global_262145.f_4102;
			break;
		
		case 9:
			return Global_262145.f_4117;
			break;
		
		case 13:
			return Global_262145.f_4121;
			break;
		
		case 12:
			return Global_262145.f_4120;
			break;
		
		case 2:
			return Global_262145.f_4112;
			break;
		
		case 14:
			return Global_262145.f_4122;
			break;
		
		case 20:
			if (func_110())
			{
				return 0;
			}
			if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 1)
			{
				return Global_262145.f_6652;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 2)
			{
				return Global_262145.f_6653;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 3)
			{
				return Global_262145.f_6654;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 4)
			{
				return Global_262145.f_6655;
			}
			else if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) == 5)
			{
				return Global_262145.f_6656;
			}
			break;
		
		case 6:
			return Global_262145.f_4115;
			break;
		
		case 22:
			return Global_262145.f_4128;
			break;
		
		case 23:
			return Global_262145.f_4129;
			break;
		
		case 24:
			return Global_262145.f_4130;
			break;
		
		case 25:
			return Global_262145.f_4131;
			break;
		
		case 26:
			return Global_262145.f_4132;
			break;
		
		case 35:
			return Global_262145.f_7221;
			break;
		
		case 15:
			return Global_262145.f_6646;
			break;
		
		case 17:
			return Global_262145.f_6646;
			break;
		
		case 18:
			return Global_262145.f_6646;
			break;
		
		case 19:
			return Global_262145.f_6646;
			break;
		
		case 21:
			return Global_262145.f_6646;
			break;
		
		case 36:
			return Global_262145.f_7688;
			break;
		
		case 39:
			return -1;
			break;
		
		case 40:
			return Global_262145.f_12884;
			break;
		
		case 41:
			return Global_262145.f_12885;
			break;
		
		case 42:
			return Global_262145.f_12886;
			break;
		
		case 43:
			return Global_262145.f_15429;
			break;
		
		case 44:
			return Global_262145.f_15431;
			break;
		
		case 57:
			return Global_262145.f_4130;
			break;
		
		case 58:
			return Global_262145.f_24998;
			break;
		
		case 62:
			return Global_262145.f_24999;
			break;
		
		case 63:
			return Global_262145.f_28457;
			break;
		
		case 64:
			return Global_262145.f_6646;
			break;
	}
	return 0;
}

void func_114(int iParam0)
{
	Global_968397 = iParam0;
}

bool func_115()
{
	return Global_1312837 == 10;
}

void func_116()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_2), 26);
}

void func_117(int iParam0, int iParam1)
{
	Global_2451426.f_3885 = 0;
	iVar0 = 0;
	while (iVar0 < 32)
	{
		Global_2451426.f_3885.f_1[iVar0] = { Var1 };
		iVar0++;
	}
	Global_2451426.f_3885.f_1[0] = { func_107(PLAYER::PLAYER_ID()) };
	Global_2451426.f_3885 = 1;
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar14 = PLAYER::INT_TO_PLAYERINDEX(iVar0);
		if ((NETWORK::NETWORK_IS_PLAYER_ACTIVE(iVar14) && iVar14 != PLAYER::PLAYER_ID()) && Global_2451426.f_3885 < iParam1)
		{
			Var1 = { func_107(iVar14) };
			if ((iParam0 || NETWORK::NETWORK_IS_FRIEND(&Var1)) && iVar0 < 32)
			{
				if (iParam0 || func_118(iVar14))
				{
					Global_2451426.f_3885.f_1[Global_2451426.f_3885] = { Var1 };
					Global_2451426.f_3885++;
				}
			}
		}
		iVar0++;
	}
}

int func_118(int iParam0)
{
	if (!func_119(iParam0) && !NETWORK::NETWORK_HAS_PLAYER_STARTED_TRANSITION(iParam0))
	{
		return 1;
	}
	return 0;
}

bool func_119(int iParam0)
{
	return Global_1590535[iParam0].f_196 != 0;
}

void func_120()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_2), 9);
}

void func_121()
{
	if (!ENTITY::IS_ENTITY_DEAD(PLAYER::PLAYER_PED_ID(), 0))
	{
		AUDIO::PLAY_SOUND_FRONTEND(-1, "Menu_Accept", &Global_19475, 1);
		func_122();
	}
}

void func_122()
{
	if (func_123())
	{
		MOBILE::_MOVE_FINGER(5);
	}
}

int func_123()
{
	if (Global_76622)
	{
		return 0;
	}
	iVar2 = 0;
	uVar0 = CAM::_0x19CAFA3C87F7C2FF();
	iVar1 = CAM::_0xEE778F8C7E1142E2(uVar0);
	if (iVar1 == 4)
	{
		iVar2 = 1;
	}
	if (Global_4270043 || iVar2)
	{
		return 1;
	}
	return 1;
}

int func_124(int iParam0)
{
	if ((iParam0 != -1 && Global_262145.f_6446[iParam0] == 129) && Global_1574577.f_25[iParam0] != -1)
	{
		iVar0 = func_111(2481, -1, 0);
		if (!GAMEPLAY::IS_BIT_SET(iVar0, 22))
		{
			return 1;
		}
	}
	return 0;
}

int func_125()
{
	return func_126(129);
}

int func_126(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 14)
	{
		if (Global_262145.f_6446[iVar0] == iParam0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

void func_127()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_3), 9);
}

void func_128()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_3), 6);
}

void func_129(int iParam0)
{
	Global_2451426.f_6204 = iParam0;
}

int func_130(bool bParam0)
{
	if (func_133() && func_131(bParam0))
	{
		if (bParam0)
		{
			return 13;
		}
		return 11;
	}
	if (bParam0)
	{
		return 12;
	}
	return 10;
}

int func_131(bool bParam0)
{
	if (bParam0)
	{
		iVar0 = 6506;
	}
	else
	{
		iVar0 = 9697;
	}
	iVar1 = func_111(iVar0, -1, 0);
	if (iVar1 == 0)
	{
		return 1;
	}
	if (NETWORK::_GET_POSIX_TIME() >= iVar1)
	{
		func_132(iVar0, 0, -1, 1, 0);
		return 1;
	}
	return 0;
}

void func_132(int iParam0, int iParam1, int iParam2, int iParam3, bool bParam4)
{
	if (bParam4)
	{
	}
	iVar0 = Global_2548434[iParam0][func_112(iParam2)];
	if (iVar0 != 0)
	{
		STATS::STAT_SET_INT(iVar0, iParam1, iParam3);
	}
}

int func_133()
{
	if (GAMEPLAY::IS_BIT_SET(GAMEPLAY::GET_RANDOM_INT_IN_RANGE(0, 65535), 0))
	{
		return 1;
	}
	return 0;
}

void func_134()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_2), 21);
}

int func_135(bool bParam0)
{
	if (bParam0)
	{
		return 233;
	}
	Var0 = { func_136() };
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(&Var0) && !GAMEPLAY::ARE_STRINGS_EQUAL(".", &Var0))
	{
		return GAMEPLAY::GET_HASH_KEY(&Var0);
	}
	if (Global_262145.f_8584 == Global_4456448.f_194990)
	{
		return Global_262145.f_8586;
	}
	return Global_4456448.f_194990;
}

struct<6> func_136()
{
	if (GAMEPLAY::IS_STRING_NULL_OR_EMPTY(&(Global_1668055.f_10)))
	{
		iVar7 = func_111(9534, -1, 0);
		if (iVar7 == 0)
		{
			StringCopy(&Var0, ".", 24);
		}
		else
		{
			iVar6 = 0;
			while (iVar6 < 1218)
			{
				if ((Global_794709.f_4[iVar6].f_65 == 0 && Global_794709.f_4[iVar6].f_68 == 1) && GAMEPLAY::IS_BIT_SET(Global_794709.f_4[iVar6].f_76, 13))
				{
					if (Global_794709.f_107189[iVar6].f_1 == iVar7)
					{
						Var0 = { Global_794709.f_123037[Global_794709.f_107189[iVar6].f_10] };
						iVar6 = 1218;
					}
				}
				iVar6++;
			}
			if (GAMEPLAY::IS_STRING_NULL_OR_EMPTY(&Var0))
			{
			}
		}
		Global_1668055.f_10 = { Var0 };
		return Var0;
	}
	return Global_1668055.f_10;
}

void func_137(int iParam0)
{
	if (iParam0 == -1)
	{
		Global_2450632.f_789++;
	}
	else
	{
		Global_2450632.f_789 = iParam0;
	}
}

void func_138()
{
	func_139();
	GAMEPLAY::SET_BIT(&(Global_2450632.f_2), 11);
}

void func_139()
{
	GAMEPLAY::SET_BIT(&(Global_1590535[PLAYER::PLAYER_ID()].f_39.f_18), 5);
}

void func_140()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_3), 5);
}

void func_141()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_3), 3);
}

int func_142(int iParam0, int iParam1)
{
	if (iParam0 > Global_262145.f_5922[iParam1])
	{
		iParam0 = 0;
	}
	return Global_262145.f_5032[iParam1][iParam0];
}

int func_143()
{
	return func_126(2);
}

int func_144(int iParam0)
{
	if (iParam0 > 50)
	{
		iParam0 = 0;
	}
	return Global_262145.f_4766[iParam0];
}

int func_145()
{
	return ((NETWORK::_GET_POSIX_TIME() / Global_262145.f_4970) % func_146());
}

int func_146()
{
	iVar0 = 0;
	while (iVar0 < 50)
	{
		if (Global_262145.f_4766[iVar0] == 0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return iVar0;
}

void func_147()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_3), 2);
}

void func_148(var uParam0)
{
	Global_2450632.f_38 = uParam0;
}

void func_149(char* sParam0)
{
	StringCopy(&(Global_2450632.f_759), sParam0, 24);
	if (func_150())
	{
		StringCopy(&(Global_959568.f_42), sParam0, 24);
	}
}

bool func_150()
{
	return GAMEPLAY::IS_BIT_SET(Global_2451426.f_1.f_2809, 5);
}

int func_151(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 1218)
	{
		if (iParam0 == Global_794709.f_107189[iVar0].f_1)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

int func_152(int iParam0)
{
	return ((NETWORK::_GET_POSIX_TIME() / Global_262145.f_6427[iParam0]) % func_153(iParam0));
}

int func_153(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < Global_262145.f_5922[iParam0])
	{
		if (Global_262145.f_5032[iParam0][iVar0] == 0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return iVar0;
}

int func_154(int iParam0)
{
	iVar0 = 0;
	while (iVar0 < 14)
	{
		if (Global_262145.f_6446[iVar0] == iParam0)
		{
			return iVar0;
		}
		iVar0++;
	}
	return -1;
}

void func_155()
{
	StringCopy(&(Global_2450632.f_759), sVar0, 24);
}

int func_156(int iParam0)
{
	switch (iParam0)
	{
		case 28:
			if (func_179())
			{
				if (func_115() || func_178())
				{
					return 0;
				}
				else
				{
					return 1;
				}
			}
			break;
		
		case 17:
			return func_170(2, 0, 0);
		
		case 18:
			if (func_115() || func_178())
			{
				return 0;
			}
			else
			{
				return func_170(1, 0, 0);
			}
			break;
		
		case 10:
			if ((func_115() || func_178()) || !Global_262145.f_6497)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 11:
			if (((func_115() || func_178()) || func_154(202) == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 12:
			if ((((func_115() || func_178()) || !Global_262145.f_6499) || Global_262145.f_6446[func_143()] != 2) || Global_1574577.f_24 == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 6:
			iVar0 = func_126(223);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 9:
			iVar0 = func_126(224);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 8:
			iVar0 = func_126(232);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 7:
			iVar0 = func_126(246);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 5:
			iVar0 = func_126(251);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 3:
			iVar0 = func_126(252);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 4:
			iVar0 = func_126(253);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 1:
			iVar0 = func_126(161);
			if (((func_115() || func_178()) || iVar0 == -1) || Global_1574577.f_25[iVar0] == -1)
			{
				return 0;
			}
			else
			{
				return func_170(2, 0, 0);
			}
			break;
		
		case 0:
			if ((func_115() || func_178()) || Global_262145.f_4739)
			{
				return 0;
			}
			else
			{
				return func_170(0, 0, 0);
			}
			break;
		
		case 15:
			if ((func_115() || func_178()) || Global_262145.f_4738)
			{
				return 0;
			}
			else
			{
				return 1;
			}
			break;
		
		case 14:
			if ((func_115() || func_178()) || Global_262145.f_4738)
			{
				return 0;
			}
			else
			{
				return 1;
			}
			break;
		
		case 16:
			if ((func_115() || func_178()) || Global_262145.f_4690 == 1)
			{
				return 0;
			}
			else
			{
				return ((func_169() || func_166()) || func_163());
			}
			break;
		
		case 2:
			if ((func_115() || func_178()) || Global_262145.f_4690 == 1)
			{
				return 0;
			}
			else
			{
				return func_170(0, 0, 0);
			}
			break;
		
		case 19:
			if (((func_115() || func_162()) || Global_794709.f_123069[9] == 0) || func_178())
			{
				return 0;
			}
			else
			{
				return func_170(0, 0, 0);
			}
			break;
		
		case 22:
			if ((func_115() || Global_794709.f_123069[7] == 0) || func_178())
			{
				return 0;
			}
			else
			{
				return func_170(0, 0, 0);
			}
			break;
		
		case 23:
			if ((func_115() || Global_794709.f_123069[4] == 0) || func_178())
			{
				return 0;
			}
			else
			{
				return func_170(4, 0, 0);
			}
			break;
		
		case 20:
			if (((func_115() || !func_161()) || func_178()) || Global_262145.f_12197)
			{
				return 0;
			}
			else
			{
				return (func_158(0) > 0 || func_157());
			}
			break;
		
		case 13:
			if ((func_115() || Global_794709.f_123069[11] == 0) || func_178())
			{
				return 0;
			}
			else
			{
				return (func_170(10, 0, 0) || func_170(0, 0, 0));
			}
			break;
		
		case 21:
			if ((func_115() || Global_794709.f_123069[10] == 0) || func_178())
			{
				return 0;
			}
			else
			{
				return func_170(0, 0, 0);
			}
			break;
		
		case 24:
			return func_170(3, 0, 0);
		
		case 25:
			return func_170(8, 0, 0);
		
		case 26:
			if (Global_262145.f_4701)
			{
				return 0;
			}
			else
			{
				return func_170(15, 0, 0);
			}
			break;
		
		case 27:
			if ((((((func_170(2, 0, 0) && func_170(1, 0, 0)) && func_170(0, 0, 0)) && func_170(3, 0, 0)) && func_170(8, 0, 0)) && !func_115()) && !func_178())
			{
				return 1;
			}
			break;
	}
	return 0;
}

bool func_157()
{
	return func_111(2931, -1, 0) > 0;
}

int func_158(int iParam0)
{
	if (Global_1671743[iParam0] == -1)
	{
		iVar0 = func_111(func_160(iParam0), -1, 0);
		if (iVar0 == -1)
		{
			func_159(iParam0, 0);
			iVar0 = 0;
		}
		Global_1671743[iParam0] = iVar0;
	}
	return Global_1671743[iParam0];
}

void func_159(int iParam0, int iParam1)
{
	Global_1671743[iParam0] = iParam1;
	func_132(func_160(iParam0), iParam1, -1, 1, 0);
}

int func_160(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return 9978;
		
		default:
	}
	return 9978;
}

bool func_161()
{
	if (Global_262145.f_12195 == 1)
	{
		return 0;
	}
	return Global_794709.f_123024[0].f_9 != 0;
}

var func_162()
{
	return Global_1573282.f_4;
}

int func_163()
{
	if (func_165(PLAYER::PLAYER_ID()))
	{
		return 1;
	}
	uVar0 = Global_1388048[func_112(-1)];
	if (GAMEPLAY::IS_BIT_SET(uVar0, 9))
	{
		func_164(1);
		return 1;
	}
	return 0;
}

void func_164(bool bParam0)
{
	if (bParam0)
	{
		if (!GAMEPLAY::IS_BIT_SET(Global_1590535[PLAYER::PLAYER_ID()].f_142, 21))
		{
			GAMEPLAY::SET_BIT(&(Global_1590535[PLAYER::PLAYER_ID()].f_142), 21);
		}
	}
	else if (GAMEPLAY::IS_BIT_SET(Global_1590535[PLAYER::PLAYER_ID()].f_142, 21))
	{
		GAMEPLAY::CLEAR_BIT(&(Global_1590535[PLAYER::PLAYER_ID()].f_142), 21);
	}
}

int func_165(int iParam0)
{
	iVar0 = iParam0;
	if (iVar0 > -1)
	{
		return GAMEPLAY::IS_BIT_SET(Global_1590535[iVar0].f_142, 21);
	}
	return 0;
}

int func_166()
{
	if (Global_262145.f_4691)
	{
		return 0;
	}
	if (func_167())
	{
		return 1;
	}
	return 0;
}

int func_167()
{
	if ((((func_168(12) && func_168(13)) && func_168(14)) && func_168(15)) && func_168(16))
	{
		return 1;
	}
	return 0;
}

int func_168(int iParam0)
{
	uVar0 = Global_1387884[iParam0];
	uVar1 = uVar0;
	if (STATS::STAT_GET_BOOL(uVar1, &uVar2, -1))
	{
		return uVar2;
	}
	return 0;
}

int func_169()
{
	Var0 = { func_136() };
	if (GAMEPLAY::ARE_STRINGS_EQUAL(&Var0, ".") || GAMEPLAY::IS_STRING_NULL_OR_EMPTY(&Var0))
	{
		return 0;
	}
	return 1;
}

bool func_170(int iParam0, bool bParam1, bool bParam2)
{
	if (Global_262145.f_7686 == 1)
	{
		if (iParam0 == 67)
		{
			return 1;
		}
		if (iParam0 == 74)
		{
			return 1;
		}
		if (func_173(PLAYER::PLAYER_ID(), 85))
		{
			if (((((iParam0 == 64 || iParam0 == 77) || iParam0 == 61) || iParam0 == 81) || iParam0 == 63) || iParam0 == 62)
			{
				return 1;
			}
		}
		if ((((((((((iParam0 == 66 || iParam0 == 116) || iParam0 == 103) || iParam0 == 104) || iParam0 == 105) || iParam0 == 119) || iParam0 == 88) || iParam0 == 75) || iParam0 == 95) || iParam0 == 65) || iParam0 == 98)
		{
			return 1;
		}
	}
	if (iParam0 < 0)
	{
		return 0;
	}
	if (iParam0 == 31)
	{
		if (Global_262145.f_4718 == 1)
		{
			return 1;
		}
	}
	if (func_172() || func_178())
	{
		return 1;
	}
	iVar0 = iParam0;
	iVar1 = (iVar0 / 32);
	iVar0 = (iVar0 % 32);
	if (bParam1)
	{
		if (iParam0 == 3)
		{
			if (func_171())
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
	}
	if (bParam2)
	{
		return 0;
	}
	return GAMEPLAY::IS_BIT_SET(Global_1574637[iVar1], iVar0);
}

int func_171()
{
	if (Global_1312448)
	{
		return 1;
	}
	if (GAMEPLAY::IS_BIT_SET(Global_2537071.f_1731, 23))
	{
		return 1;
	}
	if (func_172())
	{
		return 1;
	}
	if (func_178())
	{
		return 1;
	}
	uVar0 = Global_1388048[func_112(-1)];
	if (GAMEPLAY::IS_BIT_SET(uVar0, 7))
	{
		GAMEPLAY::SET_BIT(&(Global_2537071.f_1731), 23);
		return 1;
	}
	return 0;
}

bool func_172()
{
	return Global_1312867;
}

bool func_173(int iParam0, int iParam1)
{
	if (!func_176())
	{
		return 0;
	}
	if (func_175())
	{
		return 0;
	}
	if (iParam1 == 86)
	{
		return 1;
	}
	uVar0 = func_174(iParam1);
	iVar1 = uVar0;
	return GAMEPLAY::IS_BIT_SET(Global_1590535[iParam0].f_730, iVar1);
}

int func_174(int iParam0)
{
	switch (iParam0)
	{
		case 86:
			return 0;
		
		case 19:
			return 1;
		
		case 12:
			return 2;
		
		case 31:
			return 3;
		
		case 20:
			return 4;
		
		case 18:
			return 5;
		
		case 2:
			return 6;
		
		case 76:
			return 7;
		
		case 22:
			return 8;
		
		case 53:
			return 9;
		
		case 34:
			return 10;
		
		case 152:
			return 11;
		
		case 85:
			return 12;
		
		case 84:
			return 13;
		
		case 0:
			return 14;
		
		case 1:
			return 15;
		
		case 153:
			return 16;
		
		case 151:
			return 17;
		
		case 14:
			return 18;
		
		case 15:
			return 19;
		
		case 24:
			return 20;
		
		case 30:
			return 21;
		
		case 46:
			return 22;
		
		case 47:
			return 23;
		
		case 54:
			return 24;
		
		case 51:
			return 25;
		
		case 60:
			return 26;
		
		case 62:
			return 27;
		
		case 66:
			return 28;
		
		case 69:
			return 29;
		
		case 154:
			return 30;
		
		case 82:
			return 31;
		
		case 157:
			return 32;
		
		default:
	}
	return 1;
}

bool func_175()
{
	return GAMEPLAY::IS_BIT_SET(Global_1590535[PLAYER::PLAYER_ID()].f_146, 3);
}

int func_176()
{
	if (Global_1312448)
	{
		return 1;
	}
	if (func_172())
	{
		return 1;
	}
	if (func_178())
	{
		return 1;
	}
	return func_177(120, -1);
}

int func_177(int iParam0, int iParam1)
{
	uVar0 = Global_2583656[iParam0][func_112(iParam1)];
	if (STATS::STAT_GET_BOOL(uVar0, &uVar1, -1))
	{
		return uVar1;
	}
	return 0;
}

bool func_178()
{
	return Global_1312865;
}

int func_179()
{
	if ((((func_170(2, 0, 0) || func_170(1, 0, 0)) || func_170(0, 0, 0)) || func_170(3, 0, 0)) || func_170(8, 0, 0))
	{
		return 1;
	}
	return 0;
}

int func_180(int iParam0, var uParam1)
{
	if (CONTROLS::IS_CONTROL_JUST_RELEASED(iParam0, uParam1))
	{
		if (GAMEPLAY::IS_PC_VERSION())
		{
			if (GAMEPLAY::UPDATE_ONSCREEN_KEYBOARD() == 0 || (NETWORK::_NETWORK_IS_TEXT_CHAT_ACTIVE() && CONTROLS::_IS_INPUT_DISABLED(2)))
			{
				return 0;
			}
		}
		if (UI::IS_PAUSE_MENU_ACTIVE() || UI::IS_WARNING_MESSAGE_ACTIVE())
		{
			return 0;
		}
		else
		{
			return 1;
		}
	}
	return 0;
}

void func_181()
{
	bVar0 = false;
	if (func_191(PLAYER::PLAYER_ID()))
	{
		if (func_187(1))
		{
			StringCopy(&Local_270, "PHINVQUITBBB", 24);
		}
		else
		{
			StringCopy(&Local_270, "PHINVQUITBB", 24);
		}
		bVar0 = true;
	}
	else if (func_186(PLAYER::PLAYER_ID()))
	{
		StringCopy(&Local_270, "PHINVQUITB", 24);
		bVar0 = true;
	}
	if (func_183())
	{
		StringCopy(&Local_270, "PHINVQUIT", 24);
		bVar0 = true;
	}
	if (bVar0)
	{
	}
	func_182(&Local_270, -1);
}

void func_182(char* sParam0, int iParam1)
{
	UI::BEGIN_TEXT_COMMAND_DISPLAY_HELP(sParam0);
	UI::END_TEXT_COMMAND_DISPLAY_HELP(0, 0, 1, iParam1);
}

int func_183()
{
	if (func_184(PLAYER::PLAYER_ID(), 131) || func_184(PLAYER::PLAYER_ID(), 129))
	{
		return 1;
	}
	return 0;
}

int func_184(int iParam0, int iParam1)
{
	if (Global_1628237[iParam0] == iParam1)
	{
		return func_185(iParam0);
	}
	return 0;
}

int func_185(int iParam0)
{
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		return GAMEPLAY::IS_BIT_SET(Global_1628237[iVar0].f_1, 0);
	}
	return 0;
}

int func_186(int iParam0)
{
	if (func_185(iParam0))
	{
		return 1;
	}
	if (func_191(iParam0))
	{
		return 1;
	}
	return 0;
}

bool func_187(bool bParam0)
{
	return func_188(PLAYER::PLAYER_ID(), bParam0);
}

int func_188(int iParam0, bool bParam1)
{
	return func_189(iParam0, bParam1, 1);
}

int func_189(int iParam0, bool bParam1, int iParam2)
{
	if (iParam0 == func_32())
	{
		return 0;
	}
	if (!bParam1)
	{
		if (func_190(iParam0, iParam2))
		{
			return 0;
		}
	}
	iVar0 = Global_1628237[iParam0].f_11;
	if (iVar0 != func_32() && Global_1628237[iVar0].f_11.f_450 == iParam2)
	{
		return 1;
	}
	return 0;
}

int func_190(int iParam0, int iParam1)
{
	if (iParam0 != func_32())
	{
		if (Global_1628237[iParam0].f_11 != func_32())
		{
			if (Global_1628237[iParam0].f_11 == iParam0 && Global_1628237[iParam0].f_11.f_450 == iParam1)
			{
				return 1;
			}
		}
	}
	return 0;
}

int func_191(int iParam0)
{
	iVar0 = iParam0;
	if (iVar0 != -1)
	{
		return func_192(iParam0, 9);
	}
	return 0;
}

bool func_192(int iParam0, int iParam1)
{
	return GAMEPLAY::IS_BIT_SET(Global_1628237[iParam0].f_11.f_4, iParam1);
}

void func_193()
{
	if (!ENTITY::IS_ENTITY_DEAD(PLAYER::PLAYER_PED_ID(), 0))
	{
		AUDIO::PLAY_SOUND_FRONTEND(-1, "Menu_Back", &Global_19475, 1);
	}
}

void func_194()
{
	bLocal_279 = false;
	func_211(Global_19467, "SET_DATA_SLOT_EMPTY", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_210(Global_19467, "SET_DATA_SLOT", 18f, 0f, 0f, -1f, -1f, "CELL_208", 0, 0, 0, 0);
	bVar1 = true;
	if (iLocal_288 == iLocal_292 && !Global_262145.f_4678)
	{
		if (bLocal_281)
		{
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iVar0);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
			GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIP_F");
			GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
			GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
			iVar0++;
		}
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iVar0);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
		GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("JIPMP_HEISTQ");
		GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
		GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
		iVar0++;
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iVar0);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
		GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("JIPMP_HEISTD");
		GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
		GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
		iVar0++;
	}
	else
	{
		if (bLocal_281)
		{
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iVar0);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
			GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIP_F");
			GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
			GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
			iVar0++;
		}
		if (Global_262145.f_4676 == 0)
		{
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iVar0);
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
			if (iLocal_294[iLocal_288] == 12)
			{
				if (Global_1574577.f_24 == -2)
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIPPRP");
					bVar1 = false;
				}
				else if (Global_1574577.f_24 == -3)
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIPPRV");
					UI::ADD_TEXT_COMPONENT_SUBSTRING_TEXT_LABEL(VEHICLE::GET_DISPLAY_NAME_FROM_VEHICLE_MODEL(Global_1574577.f_56));
					bVar1 = false;
				}
				else if (Global_1574577.f_24 == 0)
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIPPRC");
					UI::ADD_TEXT_COMPONENT_SUBSTRING_TEXT_LABEL(func_208(func_209()));
					bVar1 = false;
				}
				else if (Global_1574577.f_24 == -4)
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("FMSPR_HLP9P");
					bVar1 = false;
				}
				else if (!func_206())
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIPPRN");
					UI::ADD_TEXT_COMPONENT_INTEGER(func_205());
					bVar1 = false;
				}
				else if (func_203())
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("PIM_DMAGUT");
					UI::ADD_TEXT_COMPONENT_SUBSTRING_TIME((Global_1574577.f_2 - NETWORK::_GET_POSIX_TIME()) * 1000, 14);
					bVar1 = false;
				}
				else
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIP_PR");
					UI::ADD_TEXT_COMPONENT_INTEGER(func_205());
					bVar1 = true;
				}
			}
			else
			{
				bVar1 = true;
				if (iLocal_294[iLocal_288] == 0 && !func_202())
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("PIM_DMAGUT");
					UI::ADD_TEXT_COMPONENT_SUBSTRING_TIME((func_201(-1548674197) - NETWORK::_GET_POSIX_TIME()) * 1000, 14);
					bVar1 = false;
				}
				else if (iLocal_294[iLocal_288] == 15 || iLocal_294[iLocal_288] == 14)
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("JIPMP_SVMH");
				}
				else
				{
					GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("CELL_JIP_A");
				}
			}
			bLocal_279 = !bVar1;
			GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
			GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
			iVar0++;
		}
	}
	func_211(Global_19467, "DISPLAY_VIEW", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	if ((iLocal_288 != iLocal_292 && iLocal_294[iLocal_288] == 0) && !func_202())
	{
		if (!GAMEPLAY::IS_BIT_SET(Global_2537071.f_1745, 30))
		{
			func_182("CH_QM_PHONEN", -1);
			GAMEPLAY::SET_BIT(&(Global_2537071.f_1745), 30);
		}
	}
	if (iLocal_288 == iLocal_292 && !Global_262145.f_4678)
	{
		func_198("JIPMP_QJ");
	}
	else
	{
		func_198("CELL_JIP_OCL");
	}
	if (bVar1)
	{
		func_195(13, &Local_262, 1, "", 4, &Local_258, &uLocal_336);
	}
	else
	{
		func_195(1, "", 1, "", 4, &Local_258, &uLocal_336);
	}
}

void func_195(int iParam0, char* sParam1, int iParam2, char* sParam3, int iParam4, char* sParam5, var uParam6)
{
	func_196(2, iParam0, sParam1, 0, uParam6, -1);
	func_196(1, iParam2, sParam3, 1, uParam6, 17);
	func_196(3, iParam4, sParam5, 2, uParam6, -1);
}

void func_196(int iParam0, int iParam1, char* sParam2, int iParam3, var uParam4, int iParam5)
{
	if (iParam1 == 1)
	{
		func_210(Global_19467, "SET_SOFT_KEYS", SYSTEM::TO_FLOAT(iParam0), 0f, SYSTEM::TO_FLOAT(iParam1), -1f, -1f, 0, 0, 0, 0, 0);
		GAMEPLAY::CLEAR_BIT(uParam4, iParam3);
		func_197(iParam5, 0);
		return;
	}
	if (Global_19474)
	{
		func_210(Global_19467, "SET_SOFT_KEYS", SYSTEM::TO_FLOAT(iParam0), 1f, SYSTEM::TO_FLOAT(iParam1), -1f, -1f, sParam2, 0, 0, 0, 0);
		GAMEPLAY::SET_BIT(uParam4, iParam3);
		func_197(iParam5, 1);
		return;
	}
	func_210(Global_19467, "SET_SOFT_KEYS", SYSTEM::TO_FLOAT(iParam0), 1f, SYSTEM::TO_FLOAT(iParam1), -1f, -1f, 0, 0, 0, 0, 0);
	GAMEPLAY::SET_BIT(uParam4, iParam3);
	func_197(iParam5, 1);
}

void func_197(int iParam0, bool bParam1)
{
	if (iParam0 == -1)
	{
		return;
	}
	if (bParam1)
	{
		GAMEPLAY::SET_BIT(&Global_7356, iParam0);
		return;
	}
	GAMEPLAY::CLEAR_BIT(&Global_7356, iParam0);
}

void func_198(char[4] cParam0)
{
	func_199(Global_19467, "SET_HEADER", cParam0, 0, 0, 0, 0);
}

void func_199(var uParam0, char* sParam1, var uParam2, int iParam3, int iParam4, int iParam5, int iParam6)
{
	GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(uParam0, sParam1);
	func_200(uParam2);
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam3))
	{
		func_200(iParam3);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam4))
	{
		func_200(iParam4);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam5))
	{
		func_200(iParam5);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam6))
	{
		func_200(iParam6);
	}
	GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
}

void func_200(char* sParam0)
{
	GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM(uParam0);
	GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
}

int func_201(int iParam0)
{
	uVar0 = iParam0;
	if (STATS::STAT_GET_INT(uVar0, &uVar1, -1))
	{
		return uVar1;
	}
	return 0;
}

bool func_202()
{
	return NETWORK::_GET_POSIX_TIME() >= func_201(-1548674197);
}

bool func_203()
{
	func_204();
	return NETWORK::_GET_POSIX_TIME() < Global_1574577.f_2;
}

void func_204()
{
	if (Global_1574577.f_2 == -1)
	{
		Global_1574577.f_2 = func_201(-392331778);
	}
}

var func_205()
{
	return Global_262145.f_6442;
}

int func_206()
{
	iVar0 = func_205();
	if (NETWORKCASH::NETWORK_CAN_SPEND_MONEY(iVar0, 0, 1, 0, -1, 0) || func_207(1) >= iVar0)
	{
		return 1;
	}
	return 0;
}

int func_207(bool bParam0)
{
	if (bParam0)
	{
		iVar0 = NETWORKCASH::NETWORK_GET_VC_BANK_BALANCE();
	}
	iVar0 = (iVar0 + NETWORKCASH::NETWORK_GET_VC_WALLET_BALANCE(-1));
	return iVar0;
}

char* func_208(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return "FMSTP_PRC0";
		
		case 1:
			return "FMSTP_PRC1";
		
		case 2:
			return "FMSTP_PRC2";
		
		case 3:
			return "FMSTP_PRC3";
		
		case 4:
			return "FMSTP_PRC4";
		
		case 5:
			return "FMSTP_PRC5";
		
		case 6:
			return "FMSTP_PRC6";
		
		case 7:
			return "FMSTP_PRC7";
		
		case 8:
			return "FMSTP_PRC8";
		
		case 9:
			return "FMSTP_PRC9";
		
		case 10:
			return "FMSTP_PRC10";
		
		case 11:
			return "FMSTP_PRC11";
		
		case 12:
			return "FMSTP_PRC12";
		
		case 13:
			return "FMSTP_PRC13";
		
		case 14:
			return "FMSTP_PRC14";
		
		case 15:
			return "FMSTP_PRC15";
		
		case 16:
			return "FMSTP_PRC16";
		
		case 17:
			return "FMSTP_PRC17";
		
		case 18:
			return "FMSTP_PRC18";
		
		default:
	}
	return "FMSTP_PRC14";
}

int func_209()
{
	if (Global_1574577.f_40[func_143()] != -1)
	{
		return Global_262145.f_5937[Global_1574577.f_40[func_143()]];
	}
	return -1;
}

void func_210(var uParam0, char* sParam1, float fParam2, float fParam3, float fParam4, float fParam5, float fParam6, char* sParam7, int iParam8, int iParam9, int iParam10, int iParam11)
{
	GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(uParam0, sParam1);
	GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam2));
	if (fParam3 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam3));
	}
	if (fParam4 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam4));
	}
	if (fParam5 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam5));
	}
	if (fParam6 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam6));
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(sParam7))
	{
		func_200(sParam7);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam8))
	{
		func_200(iParam8);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam9))
	{
		func_200(iParam9);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam10))
	{
		func_200(iParam10);
	}
	if (!GAMEPLAY::IS_STRING_NULL_OR_EMPTY(iParam11))
	{
		func_200(iParam11);
	}
	GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
}

void func_211(var uParam0, char* sParam1, float fParam2, float fParam3, float fParam4, float fParam5, float fParam6)
{
	GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(uParam0, sParam1);
	GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam2));
	if (fParam3 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam3));
	}
	if (fParam4 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam4));
	}
	if (fParam5 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam5));
	}
	if (fParam6 != -1f)
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(SYSTEM::ROUND(fParam6));
	}
	GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
}

int func_212()
{
	if (((((((func_115() || func_178()) || Global_262145.f_4728) || Global_262145.f_4737) || iLocal_294[iLocal_288] == 12) || iLocal_294[iLocal_288] == 15) || iLocal_294[iLocal_288] == 14) || iLocal_294[iLocal_288] == 0)
	{
		return 0;
	}
	iVar0 = 0;
	while (iVar0 < 32)
	{
		iVar14 = PLAYER::INT_TO_PLAYERINDEX(iVar0);
		if (NETWORK::NETWORK_IS_PLAYER_ACTIVE(iVar14) && iVar14 != PLAYER::PLAYER_ID())
		{
			Var1 = { func_107(iVar14) };
			if (NETWORK::NETWORK_IS_FRIEND(&Var1))
			{
				if (func_118(iVar14))
				{
					return 1;
				}
			}
		}
		iVar0++;
	}
	return 0;
}

void func_213()
{
	if (GAMEPLAY::IS_STRING_NULL_OR_EMPTY(&Local_270))
	{
		return;
	}
	if (!func_214(&Local_270))
	{
		StringCopy(&Local_270, "", 24);
		return;
	}
	UI::CLEAR_HELP(1);
	StringCopy(&Local_270, "", 24);
}

bool func_214(char[4] cParam0)
{
	UI::BEGIN_TEXT_COMMAND_IS_THIS_HELP_MESSAGE_BEING_DISPLAYED(cParam0);
	return UI::END_TEXT_COMMAND_IS_THIS_HELP_MESSAGE_BEING_DISPLAYED(0);
}

void func_215(int iParam0)
{
	if (func_220())
	{
		return;
	}
	if (Global_19664)
	{
		if (func_219())
		{
			func_217(1, 1);
		}
		else
		{
			func_217(0, 0);
		}
	}
	if (Global_19486.f_1 == 10 || Global_19486.f_1 == 9)
	{
		GAMEPLAY::SET_BIT(&Global_7357, 16);
	}
	if (AUDIO::IS_MOBILE_PHONE_CALL_ONGOING())
	{
		AUDIO::STOP_SCRIPTED_CONVERSATION(0);
	}
	Global_20805 = 5;
	if (iParam0 == 1)
	{
		GAMEPLAY::SET_BIT(&Global_7356, 30);
	}
	else
	{
		GAMEPLAY::CLEAR_BIT(&Global_7356, 30);
	}
	if (!func_216())
	{
		Global_19486.f_1 = 3;
	}
}

int func_216()
{
	if (Global_19486.f_1 == 1 || Global_19486.f_1 == 0)
	{
		return 1;
	}
	return 0;
}

void func_217(bool bParam0, bool bParam1)
{
	if (bParam0)
	{
		if (func_218(0))
		{
			Global_19664 = 1;
			if (bParam1)
			{
				MOBILE::GET_MOBILE_PHONE_POSITION(&Global_19423);
			}
			Global_19414 = { Global_19432[Global_19431] };
			MOBILE::SET_MOBILE_PHONE_POSITION(Global_19414);
		}
	}
	else if (Global_19664 == 1)
	{
		Global_19664 = 0;
		Global_19414 = { Global_19439[Global_19431] };
		if (bParam1)
		{
			MOBILE::SET_MOBILE_PHONE_POSITION(Global_19423);
		}
		else
		{
			MOBILE::SET_MOBILE_PHONE_POSITION(Global_19414);
		}
	}
}

int func_218(int iParam0)
{
	if (iParam0 == 1)
	{
		if (Global_19486.f_1 > 3)
		{
			if (GAMEPLAY::IS_BIT_SET(Global_7356, 14))
			{
				return 1;
			}
			else
			{
				return 0;
			}
		}
		else
		{
			return 0;
		}
	}
	if (SCRIPT::_GET_NUM_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(-754107665) > 0)
	{
		return 1;
	}
	if (Global_19486.f_1 > 3)
	{
		return 1;
	}
	return 0;
}

bool func_219()
{
	return GAMEPLAY::IS_BIT_SET(Global_1687687, 5);
}

bool func_220()
{
	return GAMEPLAY::IS_BIT_SET(Global_1687687, 19);
}

bool func_221()
{
	return Global_1311716[0] > 0;
}

void func_222()
{
	func_224();
	if (func_221())
	{
		func_215(0);
		bLocal_279 = false;
		return;
	}
	if (iLocal_288 < 0 || iLocal_288 >= 29)
	{
		if (Global_19486.f_1 > 3)
		{
			Global_19486.f_1 = 7;
		}
		func_259();
		iLocal_249 = 0;
		bLocal_279 = false;
		return;
	}
	if (func_2(2, Global_19456, 0))
	{
		Global_19486.f_1 = 8;
		func_259();
		iLocal_276 = 0;
		iLocal_249 = 0;
		func_193();
		bLocal_279 = false;
		return;
	}
	if ((iLocal_278 == 0 && iLocal_334 > 0) && !bLocal_279)
	{
		if (func_2(2, Global_19455, 0))
		{
			Global_19464 = 1;
			func_121();
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "GET_CURRENT_SELECTION");
			uLocal_333 = GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION();
			while (!GRAPHICS::_0x768FF8961BA904D6(uLocal_333))
			{
				SYSTEM::WAIT(0);
			}
			iLocal_293 = GRAPHICS::_0x2DE7EFA66B906036(uLocal_333);
			iLocal_249 = 3;
			Global_19486.f_1 = 8;
			if (iLocal_294[iLocal_288] == 15 || iLocal_294[iLocal_288] == 14)
			{
				func_5();
			}
			else
			{
				func_223();
			}
			bLocal_279 = false;
		}
	}
	else if (func_2(2, Global_19455, 0) && !bLocal_279)
	{
		Global_19464 = 1;
		func_193();
	}
}

void func_223()
{
	if (iLocal_288 == iLocal_292)
	{
		func_198("JIPMP_HEISTT");
	}
	else
	{
		func_198(&cLocal_250);
	}
	func_211(Global_19467, "SET_DATA_SLOT_EMPTY", SYSTEM::TO_FLOAT(13), -1082130432, -1082130432, -1082130432, -1082130432);
	func_210(Global_19467, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(13), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(12), -1f, -1f, &Local_254, 0, 0, 0, 0);
	func_211(Global_19467, "DISPLAY_VIEW", SYSTEM::TO_FLOAT(13), -1082130432, -1082130432, -1082130432, -1082130432);
	func_195(13, &Local_262, 1, "", 14, &Local_266, &uLocal_336);
}

void func_224()
{
	if (iLocal_280)
	{
		if (SYSTEM::TIMERA() > 150)
		{
			iLocal_280 = 0;
		}
	}
	if (CONTROLS::_IS_INPUT_DISABLED(2))
	{
		if (func_2(2, 181, 0))
		{
			if (iLocal_276 > 0)
			{
				iLocal_276 = (iLocal_276 - 1);
			}
			func_227();
		}
		if (func_2(2, 180, 0))
		{
			iLocal_276++;
			if (iLocal_278 == 0)
			{
				if (iLocal_276 >= iLocal_277)
				{
					iLocal_276 = 0;
				}
			}
			else
			{
				iLocal_276 = 0;
			}
			func_225();
		}
	}
	if (iLocal_280 == 0)
	{
		if (func_2(2, Global_19462, 0))
		{
			if (iLocal_276 > 0)
			{
				iLocal_276 = (iLocal_276 - 1);
			}
			func_227();
			iLocal_280 = 1;
			SYSTEM::SETTIMERA(0);
		}
		if (func_2(2, Global_19463, 0))
		{
			iLocal_276++;
			if (iLocal_278 == 0)
			{
				if (iLocal_276 >= iLocal_277)
				{
					iLocal_276 = 0;
				}
			}
			else
			{
				iLocal_276 = 0;
			}
			func_225();
			iLocal_280 = 1;
			SYSTEM::SETTIMERA(0);
		}
	}
}

void func_225()
{
	func_211(Global_19467, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(3), -1082130432, -1082130432, -1082130432, -1082130432);
	AUDIO::PLAY_SOUND_FRONTEND(-1, "Menu_Navigate", &Global_19475, 1);
	func_226();
}

void func_226()
{
	if (func_123())
	{
		if (Global_19669 == 0)
		{
			MOBILE::_MOVE_FINGER(2);
		}
		else
		{
			MOBILE::_MOVE_FINGER(1);
		}
	}
}

void func_227()
{
	func_211(Global_19467, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(1), -1082130432, -1082130432, -1082130432, -1082130432);
	AUDIO::PLAY_SOUND_FRONTEND(-1, "Menu_Navigate", &Global_19475, 1);
	func_228();
}

void func_228()
{
	if (func_123())
	{
		if (Global_19669 == 0)
		{
			MOBILE::_MOVE_FINGER(1);
		}
		else
		{
			MOBILE::_MOVE_FINGER(2);
		}
	}
}

void func_229()
{
	func_224();
	if (iLocal_278 == 0 && iLocal_334 > 0)
	{
		if (func_2(2, Global_19455, 0))
		{
			Global_19464 = 1;
			func_121();
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "GET_CURRENT_SELECTION");
			uLocal_333 = GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION();
			while (!GRAPHICS::_0x768FF8961BA904D6(uLocal_333))
			{
				SYSTEM::WAIT(0);
			}
			iLocal_289 = GRAPHICS::_0x2DE7EFA66B906036(uLocal_333);
			if (iLocal_289 > -1 && iLocal_289 < 5)
			{
				if (Global_19486.f_1 > 3)
				{
					Global_19486.f_1 = 8;
				}
				bLocal_281 = func_212();
				if ((Global_262145.f_4677 && !func_115()) && Global_262145.f_4737)
				{
					iLocal_249 = 3;
					Global_19486.f_1 = 8;
					func_223();
				}
				else if (func_230(bLocal_281))
				{
					iLocal_249 = 1;
					func_194();
				}
				else
				{
					iLocal_249 = 3;
					Global_19486.f_1 = 8;
					func_223();
				}
			}
		}
	}
	else if (func_2(2, Global_19455, 0))
	{
		Global_19464 = 1;
		func_193();
	}
}

int func_230(bool bParam0)
{
	if (func_115() || func_178())
	{
		return 0;
	}
	if (bParam0 == 0 && (Global_262145.f_4676 == 1 || iLocal_282 == 1))
	{
		return 0;
	}
	return 1;
}

void func_231()
{
	func_224();
	if (iLocal_278 == 0 && iLocal_334 > 0)
	{
		if (func_2(2, Global_19455, 0))
		{
			Global_19464 = 1;
			func_121();
			GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "GET_CURRENT_SELECTION");
			uLocal_333 = GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION();
			while (!GRAPHICS::_0x768FF8961BA904D6(uLocal_333))
			{
				SYSTEM::WAIT(0);
			}
			iLocal_288 = GRAPHICS::_0x2DE7EFA66B906036(uLocal_333);
			if (iLocal_288 > -1 && iLocal_288 < 29)
			{
				if (func_156(iLocal_294[iLocal_288]))
				{
					if (Global_19486.f_1 > 3)
					{
						Global_19486.f_1 = 8;
					}
					bLocal_281 = func_212();
					if (iLocal_288 == iLocal_290)
					{
						iLocal_249 = 2;
						func_232();
					}
					else if (iLocal_288 == iLocal_292 && !Global_262145.f_4678)
					{
						iLocal_249 = 1;
						func_194();
					}
					else if ((Global_262145.f_4677 && !func_115()) && Global_262145.f_4737)
					{
						iLocal_249 = 3;
						Global_19486.f_1 = 8;
						func_223();
						bLocal_291 = true;
					}
					else if (func_230(bLocal_281))
					{
						iLocal_249 = 1;
						func_194();
						bLocal_291 = true;
					}
					else
					{
						iLocal_249 = 3;
						Global_19486.f_1 = 8;
						func_223();
						bLocal_291 = true;
					}
				}
			}
		}
	}
	else if (func_2(2, Global_19455, 0))
	{
		Global_19464 = 1;
		func_193();
	}
}

void func_232()
{
	if (!Global_19486.f_1 == 7)
	{
		if (Global_19486.f_1 > 3)
		{
			Global_19486.f_1 = 7;
		}
	}
	func_211(Global_19467, "SET_DATA_SLOT_EMPTY", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_210(Global_19467, "SET_DATA_SLOT", 18f, 0f, 0f, -1f, -1f, "CELL_208", 0, 0, 0, 0);
	func_233();
	func_211(Global_19467, "DISPLAY_VIEW", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_198(&cLocal_250);
	if (iLocal_334 == 0)
	{
		func_195(1, "", 1, "", 4, &Local_258, &uLocal_336);
	}
	else
	{
		func_195(13, &Local_262, 1, "", 4, &Local_258, &uLocal_336);
	}
	iLocal_249 = 2;
}

void func_233()
{
	iLocal_335 = 0;
	if (iLocal_278 == 0 && func_179())
	{
		iVar0 = 0;
		while (iVar0 <= 4)
		{
			if (func_235(iVar0))
			{
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iLocal_335);
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
				GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM(func_234(iVar0));
				GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
				GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
				iLocal_324[iLocal_335] = iVar0;
				iLocal_335++;
			}
			iVar0++;
		}
	}
	else
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
		GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("JIPMP_NA");
		GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
		GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
	}
}

char* func_234(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			return "JIPMP_ARM";
		
		case 1:
			return "JIPMP_DARTS";
		
		case 2:
			return "JIPMP_GOLF";
		
		case 3:
			return "JIPMP_RANG";
		
		case 4:
			return "JIPMP_TEN";
		
		default:
	}
	return "";
}

int func_235(int iParam0)
{
	switch (iParam0)
	{
		case 0:
			if (func_115() || func_178())
			{
				return 0;
			}
			else
			{
				return 1;
			}
			break;
		
		case 4:
			if (func_115() || func_178())
			{
				return 0;
			}
			else
			{
				return 1;
			}
			break;
		
		case 1:
			return 1;
			break;
		
		case 2:
			return 1;
			break;
		
		case 3:
			return 1;
			break;
	}
	return 0;
}

void func_236(bool bParam0)
{
	if (!NETWORK::NETWORK_IS_IN_SESSION())
	{
		func_238(1);
		if (bParam0)
		{
			Global_2451426.f_1.f_2802 = 0;
		}
		func_237();
		func_295();
	}
}

void func_237()
{
	GAMEPLAY::CLEAR_BIT(&(Global_2450632.f_2), 11);
}

void func_238(bool bParam0)
{
	if (bParam0)
	{
		func_239();
		if (Global_19486.f_1 == 10 || Global_19486.f_1 == 9)
		{
			GAMEPLAY::SET_BIT(&Global_7357, 16);
		}
		Global_19486.f_1 = 1;
		if (func_218(0))
		{
			func_215(0);
		}
	}
	else if (Global_19486.f_1 == 1)
	{
		if (!Global_19486.f_1 == 0)
		{
			Global_19486.f_1 = 3;
		}
	}
}

void func_239()
{
	if (Global_19486.f_1 == 9 || Global_19486.f_1 == 10)
	{
		Global_20858 = 0;
		Global_20854 = 1;
	}
}

int func_240()
{
	if (func_242(935, &(Local_283.f_1), &(Local_283.f_2), &(Local_283.f_4), &(Global_2451426.f_1.f_2807)))
	{
		if (Local_283.f_4)
		{
			return 1;
		}
		else
		{
			if (Local_283.f_3 > 2)
			{
				func_241(1200);
				return 1;
			}
			Local_283.f_3++;
			Local_283.f_1 = 0;
			Local_283.f_2 = 0;
		}
	}
	return 0;
}

void func_241(int iParam0)
{
	Global_2451426.f_1.f_2807 = iParam0;
}

int func_242(int iParam0, var uParam1, var uParam2, var uParam3, var uParam4)
{
	iVar0 = func_258(iParam0);
	if (iVar0 == -1)
	{
	}
	else
	{
		if (!func_257())
		{
			Global_1840975.f_11[iVar0] = 1;
			Global_1840975.f_22[iVar0] = 0;
			return 1;
		}
		if ((!Global_1840975.f_11[iVar0] && Global_1840975.f_22[iVar0]) && Global_1840975.f_33[iVar0] == Global_262145.f_152)
		{
			*uParam4 = Global_1840975[iVar0];
			return 1;
		}
		else
		{
			Var1.f_3.f_1 = 4;
			Var70.f_3.f_1 = 4;
			Var139.f_19.f_1 = 4;
			Var245 = { func_107(PLAYER::PLAYER_ID()) };
			Var1 = iParam0;
			if (Var1 >= 1148 && Var1 <= 1197)
			{
				Var1.f_1 = 1;
				Var1.f_3 = 0;
			}
			else
			{
				Var1.f_1 = 5;
				Var1.f_3 = 1;
				StringCopy(&(Var1.f_3.f_1[0]), "SeasonId", 32);
				StringCopy(&(Var1.f_3.f_1[0].f_8), "", 32);
				StringIntConCat(&(Var1.f_3.f_1[0].f_8), Global_262145.f_152, 32);
			}
			switch (*uParam1)
			{
				case 0:
					if (func_246(uParam2, uParam3, &Var1, &Var245))
					{
						func_245(&Var237, &Var1);
						if (*uParam3 && STATS::_0xA0F93D5465B3094D(&Var237))
						{
							if (Var237.f_3 > 0)
							{
								STATS::_0x34770B9CE0E03B91(0, &Var139);
								if (GAMEPLAY::ARE_STRINGS_EQUAL(&(Var139.f_13), ""))
								{
									*uParam1 = 1;
									*uParam4 = 0;
								}
								else
								{
									*uParam4 = STATS::_0x88578F6EC36B4A3A(0, 0);
									*uParam1 = 1;
								}
							}
							else
							{
								*uParam1 = 1;
								*uParam4 = 0;
							}
							Global_1840975[iVar0] = *uParam4;
							Global_1840975.f_33[iVar0] = Global_262145.f_152;
							Global_1840975.f_11[iVar0] = 0;
							STATS::_0x71B008056E5692D6();
						}
						else
						{
							*uParam1 = 1;
							Global_1840975.f_11[iVar0] = 1;
						}
						Global_1840975.f_22[iVar0] = 1;
						func_243(uParam2, uParam3, &Var1);
						Var1 = { Var70 };
					}
					break;
				
				case 1:
					return 1;
					break;
				}
			}
	}
	return 0;
}

void func_243(var uParam0, var uParam1, var uParam2)
{
	*uParam0 = 0;
	*uParam1 = 0;
	Global_1835008 = 0;
	func_244(&(Global_1835008.f_1));
	STATS::LEADERBOARDS_READ_CLEAR(*uParam2, uParam2->f_1, -1);
}

void func_244(var uParam0)
{
	uParam0->f_1 = 0;
}

void func_245(var uParam0, var uParam1)
{
	*uParam0 = *uParam1;
	uParam0->f_1 = uParam1->f_1;
	uParam0->f_2 = 0;
}

int func_246(var uParam0, var uParam1, var uParam2, var uParam3)
{
	switch (*uParam0)
	{
		case 0:
			if (!func_253() && !func_249())
			{
				func_247(*uParam2);
				if (STATS::LEADERBOARDS2_READ_BY_HANDLE(uParam2, uParam3))
				{
					*uParam0++;
				}
				else
				{
					*uParam1 = 0;
					*uParam0 = 3;
				}
			}
			break;
		
		case 1:
			if (!STATS::LEADERBOARDS_READ_PENDING(*uParam2, uParam2->f_1, -1))
			{
				*uParam0++;
			}
			break;
		
		case 2:
			if (STATS::LEADERBOARDS_READ_SUCCESSFUL(*uParam2, uParam2->f_1, 0))
			{
				*uParam1 = 1;
				*uParam0++;
			}
			else
			{
				*uParam1 = 0;
				*uParam0++;
			}
			break;
		
		case 3:
			return 1;
			break;
	}
	return 0;
}

void func_247(struct<2> Param0, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12, var uParam13, var uParam14, var uParam15, var uParam16, var uParam17, var uParam18, var uParam19, var uParam20, var uParam21, var uParam22, var uParam23, var uParam24, var uParam25, var uParam26, var uParam27, var uParam28, var uParam29, var uParam30, var uParam31, var uParam32, var uParam33, var uParam34, var uParam35, var uParam36, var uParam37, var uParam38, var uParam39, var uParam40, var uParam41, var uParam42, var uParam43, var uParam44, var uParam45, var uParam46, var uParam47, var uParam48, var uParam49, var uParam50, var uParam51, var uParam52, var uParam53, var uParam54, var uParam55, var uParam56, var uParam57, var uParam58, var uParam59, var uParam60, var uParam61, var uParam62, var uParam63, var uParam64, var uParam65, var uParam66, var uParam67, var uParam68)
{
	Global_1835008 = 1;
	func_248(&(Global_1835008.f_1), 1, 0);
	Global_1835008.f_3 = Param0;
	Global_1835008.f_4 = Param0.f_1;
}

void func_248(var uParam0, bool bParam1, bool bParam2)
{
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && !bParam1)
	{
		if (!bParam2)
		{
			*uParam0 = NETWORK::GET_NETWORK_TIME();
		}
		else
		{
			*uParam0 = NETWORK::_0x89023FBBF9200E9F();
		}
	}
	else
	{
		*uParam0 = GAMEPLAY::GET_GAME_TIMER();
	}
	uParam0->f_1 = 1;
}

int func_249()
{
	if (UI::IS_PAUSE_MENU_ACTIVE() && !func_250())
	{
		return 1;
	}
	return 0;
}

bool func_250()
{
	return func_251(PLAYER::PLAYER_ID());
}

int func_251(int iParam0)
{
	switch (func_252(iParam0))
	{
		case 0:
		case 1:
		case 2:
		case 3:
		case 4:
		case 6:
		case 5:
		case 7:
		case 41:
		case 36:
		case 39:
		case 42:
			return 0;
		
		default:
	}
	return 1;
}

int func_252(int iParam0)
{
	return Global_1590535[iParam0].f_196;
}

int func_253()
{
	if (STATS::_0xA31FD15197B192BD() || Global_1835008)
	{
		func_254();
		return 1;
	}
	return 0;
}

void func_254()
{
	if (func_255(&(Global_1835008.f_1), 120000, 1))
	{
		STATS::LEADERBOARDS_READ_CLEAR(Global_1835008.f_3, Global_1835008.f_4, -1);
		Global_1835008 = 0;
		func_244(&(Global_1835008.f_1));
	}
}

int func_255(var uParam0, int iParam1, bool bParam2)
{
	if (iParam1 == -1)
	{
		return 1;
	}
	func_256(uParam0, bParam2, 0);
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && !bParam2)
	{
		if (GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam0)) >= iParam1)
		{
			return 1;
		}
	}
	else if (GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(GAMEPLAY::GET_GAME_TIMER(), *uParam0)) >= iParam1)
	{
		return 1;
	}
	return 0;
}

void func_256(var uParam0, bool bParam1, bool bParam2)
{
	if (uParam0->f_1 == 0)
	{
		if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && !bParam1)
		{
			if (!bParam2)
			{
				*uParam0 = NETWORK::GET_NETWORK_TIME();
			}
			else
			{
				*uParam0 = NETWORK::_0x89023FBBF9200E9F();
			}
		}
		else
		{
			*uParam0 = GAMEPLAY::GET_GAME_TIMER();
		}
		uParam0->f_1 = 1;
	}
}

int func_257()
{
	iVar0 = PLAYER::PLAYER_ID();
	if ((Global_1590535[iVar0].f_39.f_2 == 63 || Global_1590535[iVar0].f_39.f_2 == 62) || Global_1590535[iVar0].f_39.f_2 == 61)
	{
		return 1;
	}
	return 0;
}

int func_258(int iParam0)
{
	switch (iParam0)
	{
		case 931:
			return 0;
		
		case 932:
			return 1;
		
		case 933:
			return 2;
		
		case 934:
			return 3;
		
		case 935:
			return 4;
		
		case 936:
			return 5;
		
		case 937:
			return 6;
		
		case 938:
			return 7;
		
		case 939:
			return 8;
		
		default:
	}
	if (iParam0 >= 1148 && iParam0 <= 1197)
	{
		return 9;
	}
	return -1;
}

void func_259()
{
	if (!Global_19486.f_1 == 7)
	{
		if (Global_19486.f_1 > 3)
		{
			Global_19486.f_1 = 7;
		}
	}
	func_211(Global_19467, "SET_DATA_SLOT_EMPTY", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_210(Global_19467, "SET_DATA_SLOT", 18f, 0f, 0f, -1f, -1f, "CELL_208", 0, 0, 0, 0);
	func_260();
	func_211(Global_19467, "DISPLAY_VIEW", 18f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_198(&cLocal_250);
	if (iLocal_334 == 0)
	{
		func_195(1, "", 1, "", 4, &Local_258, &uLocal_336);
	}
	else
	{
		func_195(13, &Local_262, 1, "", 4, &Local_258, &uLocal_336);
	}
	iLocal_249 = 0;
}

void func_260()
{
	iLocal_334 = 0;
	iLocal_290 = -1;
	iLocal_292 = -1;
	if (iLocal_278 == 0 && func_179())
	{
		iVar0 = 0;
		while (iVar0 <= 28)
		{
			if (func_156(iVar0))
			{
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iLocal_334);
				if (iVar0 == 26)
				{
					iLocal_290 = iLocal_334;
				}
				if (iVar0 == 16)
				{
					iLocal_292 = iLocal_334;
				}
				GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
				GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM(func_261(iVar0));
				GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
				GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
				iLocal_294[iLocal_334] = iVar0;
				iLocal_334++;
			}
			iVar0++;
		}
	}
	else
	{
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION(Global_19467, "SET_DATA_SLOT");
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(18);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(iLocal_334);
		GRAPHICS::_PUSH_SCALEFORM_MOVIE_FUNCTION_PARAMETER_INT(0);
		GRAPHICS::_BEGIN_TEXT_COMMAND_SCALEFORM("JIPMP_NA");
		GRAPHICS::_END_TEXT_COMMAND_SCALEFORM();
		GRAPHICS::_POP_SCALEFORM_MOVIE_FUNCTION_VOID();
	}
}

char* func_261(int iParam0)
{
	switch (iParam0)
	{
		case 2:
			return "HUD_AWD_GANGOP";
		
		case 28:
			return "JIPMP_ANY";
		
		case 17:
			return "JIPMP_RACE";
		
		case 18:
			return "JIPMP_DM";
		
		case 16:
			return "JIPMP_HEIST";
		
		case 19:
			return "JIPMP_MISS";
		
		case 22:
			return "JIPMP_MISS_L";
		
		case 23:
			return "JIPMP_MISS_C";
		
		case 21:
			return "JIPMP_MISS_V";
		
		case 11:
			if (Global_262145.f_9615)
			{
				return "JIPMP_ADVSD";
			}
			else
			{
				return "JIPMP_ADVSR";
			}
			break;
		
		case 13:
			if (func_124(func_125()))
			{
				return "JIPMP_STNTAM";
			}
			else
			{
				return "JIPMP_MISS_NV";
			}
			break;
		
		case 20:
			return "JIPMP_MISS_FL";
		
		case 24:
			return "JIPMP_SURV";
		
		case 25:
			return "JIPMP_PARA";
		
		case 26:
			return "JIPMP_MINI";
		
		case 27:
			return "JIPMP_PL";
		
		case 10:
			if (Global_262145.f_9615)
			{
				return "JIPMP_STNTRD";
			}
			else
			{
				return "JIPMP_STNTR";
			}
			break;
		
		case 12:
			return "JIPMP_STNTPR";
		
		case 15:
			return "JIPMP_SVM";
		
		case 14:
			return "JIPMP_WVM";
		
		case 6:
			return "JIPMP_SVR";
		
		case 9:
			return "BNKR_SERIES";
		
		case 8:
			return "TRAN_SERIES";
		
		case 7:
			return "HOT_SERIES";
		
		case 5:
			return "ARENA_SERIES";
		
		case 3:
			return "RACE_SERIES";
		
		case 4:
			return "SURV_SERIES";
		
		case 1:
			return "OW_SERIES";
		
		case 0:
			return "CH_QM_PHONE";
	}
	return "";
}

int func_262(int iParam0)
{
	if (iParam0 == 0)
	{
		return 0;
	}
	if (func_263(iParam0) == -1)
	{
		return 0;
	}
	return 1;
}

int func_263(int iParam0)
{
	if (iParam0 == 0)
	{
		return -1;
	}
	iVar0 = 0;
	iVar0 = 0;
	while (iVar0 < 16)
	{
		if (!Global_42383[iVar0] == -1)
		{
			if (iParam0 == Global_42383[iVar0].f_1)
			{
				return iVar0;
			}
		}
		iVar0++;
	}
	return -1;
}

void func_264()
{
	Global_959568.f_8 = 0;
}

void func_265()
{
	iVar5 = -1;
	iVar7 = -1;
	iVar48 = -1;
	Global_2451426.f_5859 = { Var12 };
	iVar50 = 0;
	while (iVar50 <= 31)
	{
		Global_2451426.f_1.f_845[iVar50] = { Var12 };
		Global_2451426.f_1.f_845[iVar50].f_13 = { Var25 };
		StringCopy(&(Global_2451426.f_1.f_845[iVar50].f_29), "", 16);
		Global_2451426.f_1.f_845[iVar50].f_44 = uVar43;
		Global_2451426.f_1.f_845[iVar50].f_45 = uVar44;
		Global_2451426.f_1.f_845[iVar50].f_46 = 0;
		Global_2451426.f_1.f_845[iVar50].f_47 = uVar45;
		Global_2451426.f_1.f_845[iVar50].f_50 = uVar46;
		Global_2451426.f_1.f_845[iVar50].f_55 = uVar47;
		Global_2451426.f_1.f_845[iVar50].f_48 = 0;
		Global_2451426.f_1.f_845[iVar50].f_49 = 0;
		Global_2451426.f_1.f_845[iVar50].f_54 = 0;
		Global_2451426.f_1.f_845[iVar50].f_56 = uVar47;
		Global_2451426.f_1.f_845[iVar50].f_53 = uVar49;
		iVar51 = 0;
		while (iVar51 <= 1)
		{
			Global_2451426.f_1.f_845[iVar50].f_33[iVar51] = uVar41;
			iVar51++;
		}
		iVar51 = 0;
		while (iVar51 <= 6)
		{
			Global_2451426.f_1.f_845[iVar50].f_36[iVar51] = uVar42;
			iVar51++;
		}
		iVar50++;
	}
	iVar50 = 0;
	while (iVar50 <= 1)
	{
		Global_2451426.f_1.f_2670[iVar50] = { Var12 };
		Global_2451426.f_1.f_2670[iVar50].f_13 = { Var25 };
		Global_2451426.f_1.f_2670[iVar50].f_44 = uVar43;
		Global_2451426.f_1.f_2670[iVar50].f_45 = uVar44;
		Global_2451426.f_1.f_2670[iVar50].f_47 = uVar45;
		Global_2451426.f_1.f_2670[iVar50].f_50 = uVar46;
		Global_2451426.f_1.f_2670[iVar50].f_55 = uVar47;
		Global_2451426.f_1.f_2670[iVar50].f_56 = uVar47;
		iVar51 = 0;
		while (iVar51 <= 1)
		{
			Global_2451426.f_1.f_2670[iVar50].f_33[iVar51] = uVar41;
			iVar51++;
		}
		iVar51 = 0;
		while (iVar51 <= 6)
		{
			Global_2451426.f_1.f_2670[iVar50].f_36[iVar51] = uVar42;
			iVar51++;
		}
		iVar50++;
	}
	Global_2451426.f_1.f_2798 = uVar0;
	Global_2451426.f_1.f_2799 = uVar1;
	Global_2451426.f_1.f_2800 = uVar2;
	Global_2451426.f_1.f_2802 = uVar3;
	Global_2451426.f_1.f_2805 = uVar4;
	Global_2451426.f_1.f_2806 = 0;
	Global_2451426.f_1.f_2808 = iVar5;
	Global_2451426.f_1.f_2809 = uVar6;
	Global_2451426.f_1.f_2810 = uVar6;
	Global_2451426.f_1.f_2811 = iVar7;
	Global_2451426.f_1.f_2812 = uVar8;
	iVar52 = 0;
	while (iVar52 < 8)
	{
		Global_2451426.f_1.f_2813[iVar52] = 0;
		iVar52++;
	}
	Global_2451426.f_1.f_2825 = iVar48;
	Global_2451426.f_1.f_2822 = uVar9;
	Global_2451426.f_1.f_2823 = uVar10;
	Global_2451426.f_1.f_2824 = uVar11;
	Global_2450632.f_740 = 0;
}

void func_266()
{
	if ((!func_267() && !func_296()) && Global_2451425)
	{
		Global_4456448.f_194990 = 0;
	}
}

bool func_267()
{
	return Global_1590535[PLAYER::PLAYER_ID()].f_196 != 0;
}

void func_268()
{
	GAMEPLAY::SET_BIT(&(Global_2450632.f_2), 12);
}

void func_269()
{
	Global_968397 = -1;
}

int func_270()
{
	return Global_968397;
}

void func_271(bool bParam0)
{
	if (func_290() && !func_289())
	{
		func_237();
		func_288();
	}
	else
	{
		uVar0 = func_287();
		func_286();
		func_284(1);
		func_283();
		func_282();
		func_281();
		if (uVar0 && !bParam0)
		{
			func_147();
		}
	}
	if (Global_2458936.f_8)
	{
		func_280();
	}
	func_279();
	func_278();
	func_277();
	func_275();
	if (!func_274())
	{
		func_273();
		func_137(0);
	}
	func_288();
	func_272(0);
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		NETWORK::_0xCFEB46DCD7D8D5EB(0);
	}
}

void func_272(int iParam0)
{
	Global_2450632.f_788 = iParam0;
}

void func_273()
{
	Global_2450632.f_7 = -1;
}

bool func_274()
{
	return Global_2450632.f_637;
}

void func_275()
{
	Global_2450632.f_38 = func_276();
}

int func_276()
{
	if (Global_2457997)
	{
		return 32;
	}
	return (32 - Global_2457998);
}

void func_277()
{
	Global_2450632.f_635 = 0;
}

void func_278()
{
	Global_2450632.f_617 = 0;
}

void func_279()
{
	GAMEPLAY::CLEAR_BIT(&(Global_1590535[PLAYER::PLAYER_ID()].f_39.f_18), 15);
}

void func_280()
{
	Global_2458936.f_8 = 0;
	Global_2458936.f_7 = 0;
}

void func_281()
{
	GAMEPLAY::CLEAR_BIT(&Global_2450632, 7);
}

void func_282()
{
	GAMEPLAY::CLEAR_BIT(&Global_2450632, 6);
}

void func_283()
{
	Global_2450632 = 0;
	Global_2450632.f_2 = 0;
	Global_2450632.f_3 = 0;
}

void func_284(bool bParam0)
{
	Global_2451426.f_1.f_2798 = 0;
	Global_2451426.f_1.f_2799 = 0;
	Global_2451426.f_1.f_2824 = 0;
	Global_2451426.f_1.f_2805 = 0;
	Global_2451426.f_1.f_2806 = 0;
	Global_2451426.f_1.f_2809 = 0;
	Global_2451426.f_1.f_2810 = 0;
	Global_2451426.f_1.f_2808 = -1;
	Global_2451426.f_1.f_2811 = -1;
	iVar0 = 0;
	while (iVar0 < 8)
	{
		Global_2451426.f_1.f_2813[iVar0] = 0;
		iVar0++;
	}
	Global_2451426.f_1.f_2825 = -1;
	if (bParam0)
	{
		Global_2451426.f_1.f_2822 = -1;
		Global_2451426.f_1.f_2823 = -1;
	}
	Global_2451426.f_1.f_2844 = 0;
	func_285();
	Var1.f_33 = 2;
	Var1.f_36 = 7;
	iVar58 = 0;
	while (iVar58 <= 31)
	{
		Global_2451426.f_1.f_845[iVar58] = { Var1 };
		iVar58++;
	}
	Global_2447238.f_33 = -1;
	Global_2447238.f_34 = -1;
}

void func_285()
{
	Global_2447238.f_32 = 0;
}

void func_286()
{
	iVar13 = 0;
	while (iVar13 < 32)
	{
		Global_2448385[iVar13] = { Var0 };
		StringCopy(&(Global_2448385[iVar13].f_13), "", 64);
		StringCopy(&(Global_2448385[iVar13].f_29), "", 16);
		Global_2448385[iVar13].f_33 = 0;
		Global_2448385[iVar13].f_34 = 0;
		Global_2448385[iVar13].f_35 = 0;
		iVar13++;
	}
	Global_2448385.f_1185 = 0;
	Global_2448385.f_1186 = 0;
}

bool func_287()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632.f_3, 2);
}

void func_288()
{
	GAMEPLAY::CLEAR_BIT(&(Global_1590535[PLAYER::PLAYER_ID()].f_39.f_18), 5);
}

bool func_289()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632.f_2, 27);
}

int func_290()
{
	if (((func_294() || func_293()) || func_292()) || func_291())
	{
		return 1;
	}
	return 0;
}

bool func_291()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632, 1);
}

bool func_292()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632, 2);
}

bool func_293()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632, 20);
}

var func_294()
{
	return Global_2450632.f_598;
}

void func_295()
{
	func_213();
	SCRIPT::TERMINATE_THIS_THREAD();
}

bool func_296()
{
	return Global_2450632.f_591;
}

bool func_297()
{
	return GAMEPLAY::IS_BIT_SET(Global_2450632.f_2, 11);
}

void func_298(var uParam0)
{
	*uParam0 = 0;
}

void func_299()
{
	Global_2451426.f_1.f_2807 = 0;
}

int func_300(var uParam0, int iParam1)
{
	iVar0 = 0;
	if (!NETWORK::NETWORK_IS_SIGNED_IN())
	{
		return 12;
	}
	if (!NETWORK::NETWORK_IS_SIGNED_ONLINE())
	{
		return 12;
	}
	if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS())
	{
		if (PLAYER::GET_PLAYER_WANTED_LEVEL(PLAYER::PLAYER_ID()) > 0)
		{
			if (NETWORK::NETWORK_IS_SIGNED_ONLINE())
			{
				if (!NETWORKCASH::NETWORK_CAN_SPEND_MONEY((func_326(iParam1, iVar0, 0, -1) + func_108(20)), 0, 0, 1, -1, 0))
				{
					return 1;
				}
			}
		}
		if (func_34(PLAYER::PLAYER_ID(), 1, 1))
		{
			if (!PLAYER::IS_PLAYER_CONTROL_ON(PLAYER::PLAYER_ID()))
			{
				return 6;
			}
		}
		else
		{
			return 6;
		}
		if (!func_170(2, 0, 0))
		{
			return 4;
		}
		if (func_221())
		{
			return 6;
		}
		if (NETWORK::NETWORK_IS_SIGNED_ONLINE())
		{
			if (!NETWORKCASH::NETWORK_CAN_SPEND_MONEY(func_326(iParam1, iVar0, 0, -1), 0, 0, 1, -1, 0))
			{
				return 3;
			}
		}
		if (func_325())
		{
			return 7;
		}
		if (GAMEPLAY::IS_BIT_SET(Global_2537071.f_795, 2))
		{
			return 8;
		}
		if (((((((((((Global_1590535[PLAYER::PLAYER_ID()] == 14 || Global_1590535[PLAYER::PLAYER_ID()] == 15) || Global_1590535[PLAYER::PLAYER_ID()] == 13) || Global_1590535[PLAYER::PLAYER_ID()] == 6) || Global_1590535[PLAYER::PLAYER_ID()] == 5) || Global_1590535[PLAYER::PLAYER_ID()] == 32) || func_320()) || Global_1590535[PLAYER::PLAYER_ID()].f_732 == 1) || func_315(PLAYER::PLAYER_PED_ID(), 1215605247, 0, uParam0, 0, 500, 1, 0)) || func_314(PLAYER::PLAYER_ID(), 0)) || func_311(PLAYER::PLAYER_ID(), 1, 0)) || Global_1590535[PLAYER::PLAYER_ID()] == 148)
		{
			return 9;
		}
		if (GAMEPLAY::IS_BIT_SET(Global_2359302, 7))
		{
			return 10;
		}
		if (Global_73825 == 1)
		{
			return 11;
		}
		if ((Global_1574636 && UI::_0x4167EFE0527D706E()) && UI::IS_PAUSE_MENU_ACTIVE())
		{
			if ((Global_1574577.f_24 == -2 || Global_1574577.f_24 == -3) || Global_1574577.f_24 == 0)
			{
				return 60;
			}
			if (!Global_262145.f_6499)
			{
				return 61;
			}
			if (!func_206())
			{
				return 62;
			}
			if (Global_1574577.f_24 == -4)
			{
				return 63;
			}
			if (func_203())
			{
				return 64;
			}
		}
		else
		{
			Global_1574636 = 0;
		}
	}
	else if (func_308())
	{
		iVar1 = func_304(0, 0, 1);
		if (iVar1 == -1)
		{
			return 5;
		}
		else
		{
			if (!func_303(iVar1))
			{
				return 4;
			}
			if (NETWORK::NETWORK_IS_SIGNED_ONLINE())
			{
				if (!NETWORKCASH::NETWORK_CAN_SPEND_MONEY(func_326(iParam1, iVar0, 0, -1), 0, 0, 1, -1, 0))
				{
					return 3;
				}
			}
			if (func_325())
			{
				return 7;
			}
		}
	}
	else if (!func_301(1, 0) && !func_301(1, 1))
	{
		return 5;
	}
	else if (func_301(1, 0) && !func_301(1, 1))
	{
	}
	else if (!func_301(1, 0) && func_301(1, 1))
	{
	}
	return 0;
}

bool func_301(int iParam0, int iParam1)
{
	if (iParam1 == -1)
	{
		iParam1 = func_22();
	}
	uVar0 = func_302(iParam1);
	uVar1 = GAMEPLAY::GET_PROFILE_SETTING(uVar0);
	return GAMEPLAY::IS_BIT_SET(uVar1, iParam0);
}

int func_302(int iParam0)
{
	if (iParam0 == -1)
	{
		iParam0 = func_22();
	}
	switch (iParam0)
	{
		case 0:
			iVar0 = 914;
			break;
		
		case 1:
			iVar0 = 915;
			break;
		
		case 2:
			iVar0 = 916;
			break;
		
		case 3:
			iVar0 = 917;
			break;
		
		case 4:
			iVar0 = 918;
			break;
	}
	return iVar0;
}

int func_303(int iParam0)
{
	if (Global_1312448)
	{
		return 1;
	}
	if (func_172())
	{
		return 1;
	}
	if (func_178())
	{
		return 1;
	}
	return func_177(119, iParam0);
}

int func_304(int iParam0, int iParam1, int iParam2)
{
	iVar0 = iParam0;
	if (iParam1 == 0)
	{
	}
	iVar2 = func_22();
	if (func_307(iVar2) == 0)
	{
		iVar1 = 0;
		while (iVar1 <= 1)
		{
			iVar0 = iVar1;
			if (func_307(iVar0) && func_306(func_21(iVar0, 0)))
			{
				iVar1 = 2;
			}
			else if (iVar0 == 1)
			{
				if (iParam2 == 0)
				{
					iVar0 = 0;
				}
				else
				{
					iVar0 = -1;
				}
			}
			iVar1++;
		}
	}
	else
	{
		iVar0 = iVar2;
	}
	if (func_305(iVar0))
	{
		iVar0 = -1;
	}
	if (iVar0 == -1 && iParam2 == 0)
	{
		iVar0 = 0;
	}
	return iVar0;
}

bool func_305(int iParam0)
{
	return Global_1312905[iParam0 + 1];
}

int func_306(int iParam0)
{
	if (iParam0 == 8)
	{
		return 1;
	}
	if (iParam0 == 9)
	{
		return 1;
	}
	return 0;
}

int func_307(int iParam0)
{
	if (func_177(76, iParam0) == 1)
	{
		return 1;
	}
	return 0;
}

int func_308()
{
	if (func_310() && func_309(0))
	{
		return 1;
	}
	return 0;
}

var func_309(int iParam0)
{
	return Global_1312374[iParam0];
}

var func_310()
{
	return func_309(func_22() + 1);
}

int func_311(int iParam0, bool bParam1, bool bParam2)
{
	if (bParam1)
	{
		if (func_312(iParam0))
		{
			return 1;
		}
	}
	if (!bParam2)
	{
	}
	if (Global_1590535[iParam0] == -1)
	{
		return 0;
	}
	return 1;
}

bool func_312(int iParam0)
{
	return func_313(iParam0);
}

bool func_313(int iParam0)
{
	return GAMEPLAY::IS_BIT_SET(Global_1590535[iParam0].f_13.f_1, 0);
}

bool func_314(int iParam0, int iParam1)
{
	return GAMEPLAY::IS_BIT_SET(Global_2425662[iParam0].f_208, iParam1);
}

int func_315(int iParam0, int iParam1, bool bParam2, var uParam3, int iParam4, int iParam5, bool bParam6, int iParam7)
{
	if (!func_319(uParam3))
	{
		func_256(uParam3, 0, 0);
	}
	func_318(&Var3, iParam1);
	if (ENTITY::DOES_ENTITY_EXIST(uParam0))
	{
		if (!ENTITY::IS_ENTITY_DEAD(iParam0, 0))
		{
			if (func_316(iParam0, iParam1, 30))
			{
				if (ENTITY::IS_ENTITY_A_PED(iParam0))
				{
					uVar0 = ENTITY::GET_PED_INDEX_FROM_ENTITY_INDEX(iParam0);
					if (!PED::IS_PED_INJURED(uVar0))
					{
						if (PED::IS_PED_A_PLAYER(iVar0))
						{
							uVar2 = NETWORK::_NETWORK_GET_PED_PLAYER(iVar0);
							if (bParam2)
							{
								if (OBJECT::_0x024A60DEB0EA69F0(iParam1, uVar2, iParam4, -1))
								{
									if (iParam7 || (!OBJECT::_0x1761DC5D8471CBAA(iParam1, uVar2, 2) && !(Var3.f_7 != 0 && OBJECT::_0x1761DC5D8471CBAA(iParam1, uVar2, 3))))
									{
										if (bParam6)
										{
											return 1;
										}
										else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
										{
											return 1;
										}
										else
										{
											return 0;
										}
									}
								}
							}
							else if (OBJECT::_0x024A60DEB0EA69F0(iParam1, uVar2, iParam4, -1))
							{
								if (bParam6)
								{
									return 1;
								}
								else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
								{
									return 1;
								}
								else
								{
									return 0;
								}
							}
							else if (OBJECT::_0x1761DC5D8471CBAA(iParam1, uVar2, 2))
							{
								if (bParam6)
								{
									return 1;
								}
								else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
								{
									return 1;
								}
								else
								{
									return 0;
								}
							}
							else if (Var3.f_7 != 0 && OBJECT::_0x1761DC5D8471CBAA(iParam1, uVar2, 3))
							{
								if (bParam6)
								{
									return 1;
								}
								else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
								{
									return 1;
								}
								else
								{
									return 0;
								}
							}
						}
						else if (PED::IS_PED_IN_ANY_VEHICLE(iVar0, 0))
						{
							uVar1 = PED::GET_VEHICLE_PED_IS_IN(iVar0, 0);
							if (ENTITY::DOES_ENTITY_EXIST(uVar1))
							{
								if (bParam2)
								{
									if (OBJECT::_0x372EF6699146A1E4(iParam1, iVar1, iParam4, -1))
									{
										if (iParam7 || (!OBJECT::_0xF0EED5A6BC7B237A(iParam1, iVar1, 2) && !(Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iVar1, 3))))
										{
											if (bParam6)
											{
												return 1;
											}
											else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
											{
												return 1;
											}
											else
											{
												return 0;
											}
										}
									}
								}
								else if (OBJECT::_0x372EF6699146A1E4(iParam1, iVar1, iParam4, -1))
								{
									if (bParam6)
									{
										return 1;
									}
									else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
									{
										return 1;
									}
									else
									{
										return 0;
									}
								}
								else if (OBJECT::_0xF0EED5A6BC7B237A(iParam1, iVar1, 2))
								{
									if (bParam6)
									{
										return 1;
									}
									else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
									{
										return 1;
									}
									else
									{
										return 0;
									}
								}
								else if (Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iVar1, 3))
								{
									if (bParam6)
									{
										return 1;
									}
									else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
									{
										return 1;
									}
									else
									{
										return 0;
									}
								}
							}
						}
						else if (bParam2)
						{
							if (OBJECT::_0x372EF6699146A1E4(iParam1, iParam0, iParam4, -1))
							{
								if (iParam7 || (!OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 2) && !(Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 3))))
								{
									if (bParam6)
									{
										return 1;
									}
									else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
									{
										return 1;
									}
									else
									{
										return 0;
									}
								}
							}
						}
						else if (OBJECT::_0x372EF6699146A1E4(iParam1, iParam0, iParam4, -1))
						{
							if (bParam6)
							{
								return 1;
							}
							else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
							{
								return 1;
							}
							else
							{
								return 0;
							}
						}
						else if (OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 2))
						{
							if (bParam6)
							{
								return 1;
							}
							else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
							{
								return 1;
							}
							else
							{
								return 0;
							}
						}
						else if (Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 3))
						{
							if (bParam6)
							{
								return 1;
							}
							else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
							{
								return 1;
							}
							else
							{
								return 0;
							}
						}
					}
				}
				else if (ENTITY::IS_ENTITY_A_VEHICLE(iParam0) || ENTITY::IS_ENTITY_AN_OBJECT(iParam0))
				{
					if (bParam2)
					{
						if (OBJECT::_0x372EF6699146A1E4(iParam1, iParam0, iParam4, -1))
						{
							if (iParam7 || (!OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 2) && !(Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 3))))
							{
								if (bParam6)
								{
									return 1;
								}
								else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
								{
									return 1;
								}
								else
								{
									return 0;
								}
							}
						}
					}
					else if (OBJECT::_0x372EF6699146A1E4(iParam1, iParam0, iParam4, -1))
					{
						if (bParam6)
						{
							return 1;
						}
						else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
						{
							return 1;
						}
						else
						{
							return 0;
						}
					}
					else if (OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 2))
					{
						if (bParam6)
						{
							return 1;
						}
						else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
						{
							return 1;
						}
						else
						{
							return 0;
						}
					}
					else if (Var3.f_7 != 0 && OBJECT::_0xF0EED5A6BC7B237A(iParam1, iParam0, 3))
					{
						if (bParam6)
						{
							return 1;
						}
						else if (iParam5 <= 0 || GAMEPLAY::ABSI(NETWORK::GET_TIME_DIFFERENCE(NETWORK::GET_NETWORK_TIME(), *uParam3)) >= iParam5)
						{
							return 1;
						}
						else
						{
							return 0;
						}
					}
				}
			}
		}
	}
	func_244(uParam3);
	return 0;
}

int func_316(int iParam0, int iParam1, int iParam2)
{
	if (iParam1 != -1)
	{
		if (GAMEPLAY::GET_DISTANCE_BETWEEN_COORDS(ENTITY::GET_ENTITY_COORDS(uParam0, 1), func_317(iParam1), 1) <= IntToFloat(iParam2))
		{
			return 1;
		}
	}
	return 0;
}

Vector3 func_317(int iParam0)
{
	switch (iParam0)
	{
		case 1215605247:
			return 1204.429f, -3110.847f, 4.3988f;
			break;
		
		case -1710530912:
			return 725.1831f, -1089.349f, 21.1692f;
			break;
		
		case 1131590004:
			return -1164.887f, -2011.105f, 12.25371f;
			break;
		
		case 916723671:
			return -330.44f, -143.39f, 39.33f;
			break;
		
		case 1340820069:
			return 106.28f, 6620.01f, 32.12f;
			break;
		
		case -866958545:
			return 1182.65f, 2641.9f, 38.05f;
			break;
	}
	return 0f, 0f, 0f;
}

void func_318(var uParam0, int iParam1)
{
	switch (iParam1)
	{
		case 0:
		case 1215605247:
			*uParam0 = 99;
			uParam0->f_1 = 1215605247;
			uParam0->f_2 = 0;
			uParam0->f_3 = { 1204.429f, -3110.847f, 4.3988f };
			uParam0->f_6 = -247372382;
			uParam0->f_7 = 0;
			uParam0->f_8 = "MP_GAR_SIMEON";
			uParam0->f_9.f_1 = { 1210.884f, -3122.402f, 5.2118f };
			uParam0->f_9.f_4 = { 4.0534f, 0f, 32.6363f };
			uParam0->f_9.f_7 = 32.498f;
			uParam0->f_9.f_8 = { 1210.884f, -3122.402f, 5.2118f };
			uParam0->f_9.f_11 = { 4.0534f, 0f, 32.6363f };
			uParam0->f_9.f_14 = 32.498f;
			uParam0->f_27 = { 1204.157f, -3122.599f, 3.795331f };
			uParam0->f_27.f_3 = { 1204.24f, -3099.772f, 8.400777f };
			uParam0->f_27.f_6 = 7f;
			return;
			break;
		
		case 1:
		case -1710530912:
			*uParam0 = 99;
			uParam0->f_1 = -1710530912;
			uParam0->f_2 = 1;
			uParam0->f_3 = { 725.1831f, -1089.349f, 21.1692f };
			uParam0->f_7 = 0;
			uParam0->f_8 = "MP_GAR_PNS_2";
			uParam0->f_9.f_1 = { 734.3793f, -1078.791f, 23.4305f };
			uParam0->f_9.f_4 = { -16.432f, 0f, -19.7978f };
			uParam0->f_9.f_7 = 60.0199f;
			uParam0->f_9.f_8 = { 734.3027f, -1079.004f, 23.4973f };
			uParam0->f_9.f_11 = { -16.432f, 0f, -19.7978f };
			uParam0->f_9.f_14 = 60.0199f;
			uParam0->f_27 = { 738.8857f, -1088.516f, 20.55957f };
			uParam0->f_27.f_3 = { 718.613f, -1088.78f, 24.83263f };
			uParam0->f_27.f_6 = 7f;
			return;
			break;
		
		case 2:
		case 1131590004:
			*uParam0 = 99;
			uParam0->f_1 = 1131590004;
			uParam0->f_2 = 2;
			uParam0->f_3 = { -1164.887f, -2011.105f, 12.25371f };
			uParam0->f_7 = 0;
			uParam0->f_8 = "MP_GAR_NEUT_PNS_3";
			uParam0->f_9.f_1 = { -1161.774f, -2010.27f, 14.2468f };
			uParam0->f_9.f_4 = { -17.3415f, 0f, 113.6889f };
			uParam0->f_9.f_7 = 64.5334f;
			uParam0->f_9.f_8 = { -1161.639f, -2010.211f, 14.2928f };
			uParam0->f_9.f_11 = { -17.3415f, 0f, 113.6889f };
			uParam0->f_9.f_14 = 64.5334f;
			uParam0->f_27 = { -1169.723f, -2015.923f, 11.50441f };
			uParam0->f_27.f_3 = { -1160.558f, -2007.005f, 15.68027f };
			uParam0->f_27.f_6 = 5.5f;
			return;
			break;
		
		case 3:
		case 916723671:
			*uParam0 = 99;
			uParam0->f_1 = 916723671;
			uParam0->f_2 = 3;
			uParam0->f_3 = { -330.44f, -143.39f, 39.33f };
			uParam0->f_7 = 0;
			uParam0->f_8 = "MP_GAR_PNS_4";
			uParam0->f_9.f_1 = { -332.1567f, -141.0546f, 40.2864f };
			uParam0->f_9.f_4 = { -20.6629f, 0f, -134.7887f };
			uParam0->f_9.f_7 = 60.0241f;
			uParam0->f_9.f_8 = { -332.3621f, -140.8507f, 40.3956f };
			uParam0->f_9.f_11 = { -20.6629f, 0f, -134.7887f };
			uParam0->f_9.f_14 = 60.0241f;
			uParam0->f_27 = { -323.7998f, -146.2539f, 37.81492f };
			uParam0->f_27.f_3 = { -334.3432f, -141.7261f, 40.75964f };
			uParam0->f_27.f_6 = 5.5f;
			return;
			break;
		
		case 4:
		case 1340820069:
			*uParam0 = 99;
			uParam0->f_1 = 1340820069;
			uParam0->f_2 = 4;
			uParam0->f_3 = { 106.28f, 6620.01f, 32.12f };
			uParam0->f_7 = 0;
			uParam0->f_8 = "MP_GAR_PNS_5";
			uParam0->f_9.f_1 = { 106.688f, 6617.322f, 32.5026f };
			uParam0->f_9.f_4 = { -10.7437f, 0f, 21.7154f };
			uParam0->f_9.f_7 = 67.562f;
			uParam0->f_9.f_8 = { 106.7971f, 6617.048f, 32.5586f };
			uParam0->f_9.f_11 = { -10.7437f, 0f, 21.7154f };
			uParam0->f_9.f_14 = 67.562f;
			uParam0->f_27 = { 100.9759f, 6625.046f, 30.60301f };
			uParam0->f_27.f_3 = { 111.2522f, 6615.657f, 33.62929f };
			uParam0->f_27.f_6 = 5.5f;
			return;
			break;
		
		case 5:
		case -866958545:
			*uParam0 = 99;
			uParam0->f_1 = -866958545;
			uParam0->f_2 = 5;
			uParam0->f_3 = { 1182.65f, 2641.9f, 38.05f };
			uParam0->f_7 = 0;
			uParam0->f_9.f_1 = { 1184.206f, 2644.004f, 38.7458f };
			uParam0->f_9.f_4 = { -15.4014f, 0f, 161.4493f };
			uParam0->f_9.f_7 = 67.3374f;
			uParam0->f_9.f_8 = { 1184.345f, 2644.418f, 38.866f };
			uParam0->f_9.f_11 = { -15.4014f, 0f, 161.4493f };
			uParam0->f_9.f_14 = 67.3374f;
			uParam0->f_27 = { 1182.835f, 2634.775f, 36.55006f };
			uParam0->f_27.f_3 = { 1182.578f, 2647.955f, 39.58602f };
			uParam0->f_27.f_6 = 5.5f;
			return;
			break;
	}
}

bool func_319(var uParam0)
{
	return uParam0->f_1;
}

int func_320()
{
	if (func_314(PLAYER::PLAYER_ID(), 8))
	{
		return 1;
	}
	if (func_314(PLAYER::PLAYER_ID(), 10))
	{
		return 1;
	}
	if (func_314(PLAYER::PLAYER_ID(), 12))
	{
		return 1;
	}
	if (func_314(PLAYER::PLAYER_ID(), 14))
	{
		return 1;
	}
	if (func_314(PLAYER::PLAYER_ID(), 13))
	{
		return 1;
	}
	if (func_324())
	{
		return 1;
	}
	if (func_323())
	{
		return 1;
	}
	if (!func_172() && !func_178())
	{
		if (!func_322())
		{
			if (!func_321())
			{
				return 1;
			}
		}
	}
	return 0;
}

int func_321()
{
	if (GAMEPLAY::IS_BIT_SET(Global_2537071.f_1731, 7))
	{
		return 1;
	}
	uVar0 = Global_1388048[func_112(-1)];
	if (GAMEPLAY::IS_BIT_SET(uVar0, 6))
	{
		GAMEPLAY::SET_BIT(&(Global_2537071.f_1731), 7);
		return 1;
	}
	if (func_172())
	{
		return 1;
	}
	if (func_178())
	{
		return 1;
	}
	return 0;
}

bool func_322()
{
	return GAMEPLAY::IS_BIT_SET(Global_1590535[PLAYER::PLAYER_ID()].f_142, 2);
}

int func_323()
{
	if (GAMEPLAY::IS_BIT_SET(Global_2537071.f_1731, 28) && !GAMEPLAY::IS_BIT_SET(Global_2537071.f_1731, 29))
	{
		return 1;
	}
	return 0;
}

bool func_324()
{
	return GAMEPLAY::IS_BIT_SET(Global_2537071.f_1732, 3);
}

bool func_325()
{
	return Global_98796.f_346 > 0;
}

int func_326(int iParam0, int iParam1, bool bParam2, int iParam3)
{
	if (Global_2391005 && !bParam2)
	{
		return 0;
	}
	if (func_327())
	{
		return 0;
	}
	if (func_312(PLAYER::PLAYER_ID()))
	{
		return Global_262145.f_6649;
	}
	switch (iParam0)
	{
		case 0:
			switch (iParam1)
			{
				case 5:
					return Global_262145.f_4442;
				
				case 6:
					return Global_262145.f_4443;
				
				case 4:
					return Global_262145.f_4444;
				
				case 9:
					return Global_262145.f_4445;
				
				default:
			}
			return 0;
		
		case 1:
			return Global_262145.f_4440;
		
		case 3:
			return Global_262145.f_4439;
		
		case 6:
			return 500;
		
		case 8:
			if (iParam3 == 1)
			{
				return 0;
			}
			else
			{
				return Global_262145.f_4441;
			}
			break;
		
		case 2:
			if (iParam3 == 1)
			{
				return Global_262145.f_2343;
			}
			else
			{
				switch (Global_4456448.f_75705)
				{
					case 4:
					case 5:
						return (Global_262145.f_4436 + Global_262145.f_2343);
					
					case 2:
					case 3:
						return (Global_262145.f_4437 + Global_262145.f_2343);
					
					case 12:
					case 13:
						return (Global_262145.f_4438 + Global_262145.f_2343);
					
					default:
				}
				return (Global_262145.f_4435 + Global_262145.f_2343);
			}
			break;
	}
	return 0;
}

bool func_327()
{
	return GAMEPLAY::IS_BIT_SET(Global_4456448.f_226020, 12);
}

